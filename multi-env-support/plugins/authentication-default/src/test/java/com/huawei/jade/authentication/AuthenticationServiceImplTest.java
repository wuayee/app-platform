...
public class AuthenticationServiceImplTest {
    private static final String TOKEN_KEY = "X-Uni-Crsf-Token";

    private static final String TOKEN_ID_KEY = "X-Uni-Crsf-Token-Id";


    @Test
    void testGetUserName_CookiesNotExist() {
        ...
        assertEquals("Jade", userName);
    }

    @Test
    void testGetUserName_CookiesExist() {
        HttpClassicServerRequest request = Mockito.mock(HttpClassicServerRequest.class);
        Cookie tokenCookie = Cookie.builder().name(TOKEN_KEY).value("tokenValue").build();
        Cookie tokenIdCookie = Cookie.builder().name(TOKEN_ID_KEY).value("tokenIdValue").build();
        DefaultCookieCollection cookies = new DefaultCookieCollection();
        cookies.add(tokenCookie);
        cookies.add(tokenIdCookie);
