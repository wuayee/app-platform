...
import com.huawei.jade.oms.util.parser.support.DefaultJsonParser;
...
/**
 * {@link DefaultRegisterProcessor} 的单元测试类。
 *
 * @author 鲁为
 * @since 2024-11-19
 */
@DisplayName("测试 RegisterProcessorImpl")
@ExtendWith(MockitoExtension.class)
public class RegisterProcessorImplTest {
    @Mock
    ...
    private DefaultRegisterProcessor processor;

    @BeforeEach
    void setup() {
        OmsClient omsClient = mock(OmsClient.class);
        this.jsonParser = new DefaultJsonParser(this.serializer);
        this.processor = new DefaultRegisterProcessor(omsClient, this.jsonParser);
    ...
    void shouldSuccessWhenRegister() {
        List<RoleI18nInfo> infoList = this.processor.parse(this.resource, RoleI18nInfo.class);
