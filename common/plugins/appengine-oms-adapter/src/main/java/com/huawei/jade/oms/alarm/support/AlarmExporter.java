/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2024-2024. All rights reserved.
 */

package com.huawei.jade.oms.alarm.support;

import static com.huawei.jade.oms.alarm.util.Constants.ALARM_CATEGORY;
import static com.huawei.jade.oms.alarm.util.Constants.ALARM_DEVICE_ID;
import static com.huawei.jade.oms.alarm.util.Constants.ALARM_EVENT_TYPE;
import static com.huawei.jade.oms.alarm.util.Constants.AUTO_CLEAR;
import static com.huawei.jade.oms.alarm.util.Constants.CLEARED;
import static com.huawei.jade.oms.alarm.util.Constants.EVENT_SUBJECT_TYPE;
import static com.huawei.jade.oms.alarm.util.Constants.SERVICE_NAME;
import static com.huawei.jade.oms.alarm.util.Constants.SERVICE_NAME_EN;
import static com.huawei.jade.oms.alarm.util.Constants.SERVICE_NAME_ZH;
import static com.huawei.jade.oms.alarm.util.Constants.UNCLEARED;
import static com.huawei.jade.oms.alarm.util.Constants.WARNING;

import com.huawei.fitframework.annotation.Component;
import com.huawei.fitframework.annotation.Fit;
import com.huawei.fitframework.annotation.Initialize;
import com.huawei.fitframework.annotation.Value;
import com.huawei.fitframework.exception.FitException;
import com.huawei.fitframework.log.Logger;
import com.huawei.fitframework.schedule.annotation.Scheduled;
import com.huawei.fitframework.serialization.ObjectSerializer;
import com.huawei.fitframework.util.IoUtils;
import com.huawei.fitframework.util.MapBuilder;
import com.huawei.fitframework.util.TypeUtils;
import com.huawei.jade.oms.alarm.enums.PvcEnum;
import com.huawei.jade.oms.alarm.service.AlarmClient;
import com.huawei.jade.oms.alarm.vo.EventDefinition;
import com.huawei.jade.oms.alarm.vo.EventInfo;
import com.huawei.jade.oms.alarm.vo.RegisterEventDefinitionReq;

import java.io.IOException;
import java.lang.reflect.Type;
import java.util.List;
import java.util.Map;
import java.util.Objects;
import java.util.stream.Collectors;

/**
 * 表示告警输出器。
 *
 * @author 何嘉斌
 * @since 2024-12-05
 */
@Component
public class AlarmExporter {
    private static final Logger log = Logger.get(AlarmExporter.class);
    private final AlarmClient client;
    private final MetricsCollector collector;
    private final ObjectSerializer serializer;

    @Value("${worker.host}")
    private String envIp;

    public AlarmExporter(AlarmClient client, MetricsCollector collector,
            @Fit(alias = "json") ObjectSerializer serializer) {
        this.client = client;
        this.collector = collector;
        this.serializer = serializer;
    }

    @Initialize
    void init() {
        List<EventDefinition> defineList = parseAlarmDefinition();
        RegisterEventDefinitionReq defineReq = new RegisterEventDefinitionReq();
        defineReq.setServiceName(SERVICE_NAME);
        defineReq.setServiceEn(SERVICE_NAME_EN);
        defineReq.setServiceZh(SERVICE_NAME_ZH);
        defineReq.setEventDefines(defineList);
        boolean result = this.client.registerEventDefinition(defineReq);
        if (!result) {
            log.error("Alarm definition registration failed.");
        }
    }

    private List<EventDefinition> parseAlarmDefinition() {
        try {
            String enContents = IoUtils.content(EventDefinition.class, "/alarm/alarm_definition_en-US.json");
            String zhContents = IoUtils.content(EventDefinition.class, "/alarm/alarm_definition_zh-CN.json");
            Map<String, String> resourceMapping =
                    MapBuilder.<String, String>get().put("en", enContents).put("zh", zhContents).build();
            return resourceMapping.entrySet().stream().flatMap(entry -> {
                List<EventDefinition> eventDefines = this.serializer.deserialize(entry.getValue(),
                        TypeUtils.parameterized(List.class, new Type[] {EventDefinition.class}));
                eventDefines.forEach(define -> {
                    define.setLanguage(entry.getKey());
                    define.setParts(ALARM_CATEGORY);
                    define.setDefineMatchKey(SERVICE_NAME + "_" + define.getEventId());
                });
                return eventDefines.stream();
            }).collect(Collectors.toList());
        } catch (IOException ex) {
            throw new FitException("Failed to load alarm definitions.", ex);
        }
    }

    @Scheduled(strategy = Scheduled.Strategy.FIXED_RATE, initialDelay = 10000L, value = "3600000")
    void checkStorageUsage() {
        Map<String, Integer> usages = this.collector.getStorageUsage();
        List<EventInfo> eventInfos = usages.entrySet()
                .stream()
                .map(entry -> validateUsage(entry.getKey(), entry.getValue()))
                .filter(Objects::nonNull)
                .collect(Collectors.toList());
        boolean result = this.client.sendEvents(eventInfos);
        if (!result) {
            log.error("Send alarm failed.");
        }
    }

    private EventInfo validateUsage(String name, Integer usage) {
        PvcEnum pvc = PvcEnum.getByPath(name);
        Integer threshold = pvc.getThreshold();
        if (threshold == null) {
            return null;
        }
        if (usage >= threshold) {
            log.warn("The filesystem {} usage exceeds threshold, current usage: {}%.", pvc, usage);
        }
        String status = usage >= threshold ? UNCLEARED : CLEARED;
        EventInfo info = new EventInfo();
        info.setEventId(pvc.getId());
        info.setEventCategory(SERVICE_NAME);
        info.setParts(ALARM_CATEGORY);
        info.setDeviceType(EVENT_SUBJECT_TYPE);
        info.setDeviceId(ALARM_DEVICE_ID);
        info.setEventSubject(pvc.getName());
        info.setEventSubjectType(EVENT_SUBJECT_TYPE);
        info.setStatus(status);
        info.setSeverity(WARNING);
        info.setEventType(ALARM_EVENT_TYPE);
        info.setEventSource(this.envIp);
        info.setShouldSaveDefine(false);
        info.setDefineMatchKey(SERVICE_NAME + "_" + info.getEventId());
        info.setClearType(AUTO_CLEAR);
        String currentTime = String.valueOf(System.currentTimeMillis());
        if (status.equals(CLEARED)) {
            info.setClearTime(currentTime);
        }
        info.setEventTimestamp(currentTime);
        return info;
    }
}