...
package com.huawei.jade.voice.to.text.controller;
...
public class VoiceController {
    ...
    public Result<String> getText(@RequestQuery(value = "voicePath", required = false) String voicePath,
                                  @RequestQuery(value = "fileName", required = false) String fileName) {
        return Result.ok(voiceService.getText(voicePath, fileName), "success", 1);
    }
     ...
     */
    @GetMapping("/toVoice")
    public Result<String> getVoice(@RequestQuery(value = "text", required = false) String text, @RequestQuery(value =
            "tone", required = false) int tone) {
