...
public class EvalDataServiceImplTest {
    ...
    void shouldFailWhenVerifyError() {
        doThrow(new AppEvalException(AppEvalRetCodeEnum.EVAL_DATA_INVALID_ERROR, "a", "b")).when(evalDataValidator)
                .verify(anyLong(), anyList());
        assertThatThrownBy(() -> evalDataService.insertAll(1L, TEST_CONTENTS)).isInstanceOf(AppEvalException.class);
    }
}