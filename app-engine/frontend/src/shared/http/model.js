...
export function getModelList(params) {
  return get(`http://80.11.128.70:8000/v1/list_supported_models`, params);
}
// 创建应用
...
export function aippStart(tenantId, appId, versionId, params) {
  return post(
    `${AIPP_URL}/${tenantId}/aipp/${appId}?version=${versionId}`,
    params
  );
}
// 发布应用
...
export function reGetInstance(tenantId, appId, instanceId, version) {
  return get(
    `${AIPP_URL}/${tenantId}/aipp/${appId}/instances/${instanceId}?version=${version}`
  );
}
// 获取历史对话
...
export function queryInspirationSelect(tenantId, fitableid, params) {
  return post(
    `${AIPP_URL}/${tenantId}/genericables/fitables/${fitableid}`,
    params
  );
}
// 文件上传
export function uploadChatFile(tenantId, appId, data, headers) {
  return post(`${AIPP_URL}/${tenantId}/file?aipp_id=${appId}`, data, {
    headers,
  });
}
// 图片预览
