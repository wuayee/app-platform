...
export function modelbaseSyncStatus() {
  const url = `${MODEL_BASE_URL}/v1/models/sync_status`;
  return get(url);
}

//查询当前模型仓最新的任务刷新状态
export function modelbaseSync() {
  const url = `${MODEL_BASE_URL}/v1/models/sync`;
  return post(url, {});
}
...
export function deleteModelbaseVersion(versionId: string) {
  const url = `${MODEL_BASE_URL}/v1/versions/${versionId}`;
  return del(url);
}

export function getModelSeries() {
  const url = `${MODEL_BASE_URL}/v1/models/seriesNames`;
  return get(url);
}
