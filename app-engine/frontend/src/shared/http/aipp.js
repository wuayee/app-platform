...
// 获取历史对话
export function getRecentInstances(tenantId, appId, version) {
  return get(`${AIPP_URL}/${tenantId}/log/aipp/${appId}/recent?version=${version}`)
}
// 删除历史对话记录
export function clearInstance(tenantId, appId, version) {
  return del(`${AIPP_URL}/${tenantId}/log/aipp/${appId}?version=${version}`)
}
// 终止当前对话
