 ...
 * @since 2024/10/15
 */
class FlowHttpJoberTest {
    private BrokerClient brokerClient;

    private FlowHttpJober target;

    @BeforeEach
    void before() {
        brokerClient = mock(BrokerClient.class);

        target = new FlowHttpJober();
    ...
    @Test
    @DisplayName("执行HttpJober调用并抛异常")
    void testHttpJoberFailed() {
        ...
        Assertions.assertEquals(ErrorCodes.FLOW_HTTP_JOBER_INVOKE_ERROR.getErrorCode(), exception.getCode());
    }
}