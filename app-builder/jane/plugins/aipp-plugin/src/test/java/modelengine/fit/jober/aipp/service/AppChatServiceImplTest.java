...
public class AppChatServiceImplTest {
    ...
    void testChat() {
        ...
        Mockito.when(this.aippRunTimeService.createInstanceByApp(Mockito.eq(atChatAppId), Mockito.eq("你好"),
                Mockito.anyMap(), Mockito.any(), Mockito.eq(false))).thenReturn(Tuple.duet("hello_inst", t2));
        ...
        Mockito.when(
                        this.metaService.list(Mockito.any(MetaFilter.class), Mockito.eq(true), Mockito.eq(0L), Mockito.eq(1),
                                Mockito.any(OperationContext.class), Mockito.any(MetaFilter.class)))
                .thenReturn(new RangedResultSet<>(Collections.singletonList(mockMeta()), new RangeResult(0, 10, 1)));
        Mockito.when(this.flowsService.getFlows(Mockito.any(), Mockito.any()))
                .thenReturn(mockFlowInfo(new ArrayList<>(), false));
        Choir<Object> objectChoir = Assertions.assertDoesNotThrow(
    ...
    void testChatWithInvalidQuestion() {
        ...
        Mockito.when(this.appFactory.create(Mockito.any()))
                .thenReturn(
                        AppBuilderApp.builder().id("id1").version("10.0.0").state("ACTIVE").type(APP.code()).build());
        Mockito.when(this.appRepository.selectWithId(Mockito.any()))
                .thenReturn(
                        AppBuilderApp.builder().id("id1").version("10.0.0").state("ACTIVE").type(APP.code()).build());
    ...
    class TestAddUserContext {
        ...
        void setup() {
            ...
            Mockito.when(metaService1.list(Mockito.any(MetaFilter.class), Mockito.anyBoolean(), Mockito.eq(0L),
                    ...
                    .thenAnswer((invocation) -> {
                        ...
                        if (argument) {
                            return new RangedResultSet<>(Collections.singletonList(mockMeta()),
                                    new RangeResult(0, 1, 1));
                        } else {
                            return new RangedResultSet<>(Collections.singletonList(mockMeta()),
                                    new RangeResult(0, 10, 1));
                        }
                    });
            CacheUtils.clear();
        }
        ...
        public void testNotRequiredParamWithNull() throws NoSuchMethodException {
            ...
            Method method = AppChatServiceImpl.class.getDeclaredMethod("addUserContext", CreateAppChatRequest.class,
                    Map.class, boolean.class, OperationContext.class, String.class);
            method.setAccessible(true);
            CreateAppChatRequest request = new CreateAppChatRequest();
            getCreateAppChatRequest(request, "input1", null);
            request.setAppId("APPID");
            Assertions.assertDoesNotThrow(
                    () -> method.invoke(this.chatService, request, new HashMap<>(), false, new OperationContext(),
                            "workflow"));
        }
        ...
        public void testValidCombinationSituation() throws NoSuchMethodException {
            List<String> names = Arrays.asList("not_required_input", "string_input", "string_input_2", "integer_input",
                    "integer_input_2", "boolean_input", "boolean_input_2", "number_input", "number_input_2",
                    "number_input_3");
            List<String> types = Arrays.asList("String", "String", "String", "Integer", "Integer", "Boolean", "Boolean",
                    "Number", "Number", "Number");
            List<Boolean> requiredStates = Arrays.asList(false, true, false, true, false, true, false, true, false,
                    true);
