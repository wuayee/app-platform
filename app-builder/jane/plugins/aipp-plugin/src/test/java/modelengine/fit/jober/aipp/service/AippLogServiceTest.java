...
public class AippLogServiceTest {
    ...
    static Stream<AippLogCreateDto> invalidAippLogCreateDtoCreatorForTest() {
        return Stream.of(AippLogCreateDto.builder()
                ...
                .build(), AippLogCreateDto.builder()
                .aippId(DUMMY_ID)
                .instanceId(null)
                .logData(DUMMY_LOG_MSG)
                .logType(AippInstLogType.MSG.name())
    ...
    private void mockMta() {
        ...
        when(this.metaServiceMock.list(any(MetaFilter.class), eq(false), eq(0L), eq(10), any(OperationContext.class),
                any(MetaFilter.class))).thenReturn(
                RangedResultSet.create(Collections.singletonList(expectMeta), 0L, 10, 1L));
    }
    ...
    void shouldDeleteWhenCallDeleteLogWithLastNotRunning() {
        ...
        metaInstanceResult.setResults(Collections.singletonList(
                new Instance(dummyInstId, Collections.singletonMap(AippConst.INST_STATUS_KEY, dummyAippInstanceStatus),
                        null)));
        ...
        when(this.metaInstanceServiceMock.list(eq(DUMMY_ID),
                argThat(filter -> filter.getIds().size() == 1 && filter.getIds().get(0).equals(dummyInstId)), eq(0L),
                eq(1), argThat(OperationContextDummy::operationContextDummyMatcher))).thenReturn(metaInstanceResult);
        ...
        verify(aippLogMapperMock, times(1)).delete(eq(Collections.singletonList(DUMMY_ID)),
                eq(AippTypeEnum.getType(AippTypeEnum.NORMAL.name()).type()), eq(OperationContextDummy.DUMMY_W3_ACCOUNT),
                isNull());
    }
    ...
    void shouldNotDeleteWhenCallDeleteLogWithLastRunning() {
        ...
        verify(metaInstanceServiceMock, times(1)).list(any(), any(), anyLong(), anyInt(), any());
        verify(aippLogMapperMock, times(1)).delete(eq(Collections.singletonList(DUMMY_ID)),
                eq(AippTypeEnum.NORMAL.name()), eq(OperationContextDummy.DUMMY_W3_ACCOUNT), eq(dummyInstId));
    ...
    void shouldReturnAllWhenCallQueryInstanceLogSinceWithInvalidTimeString(String datetimeString) {
        when(aippLogMapperMock.selectWithCondition(
                argThat(cond -> cond.getInstanceId().equals(DUMMY_ID) && Objects.isNull(
                        cond.getAfterAt())))).thenReturn(
                Stream.of(AippInstLogType.MSG, AippInstLogType.FORM, AippInstLogType.ERROR, AippInstLogType.MSG,
                                AippInstLogType.QUESTION, AippInstLogType.HIDDEN_QUESTION, AippInstLogType.HIDDEN_MSG,
                                AippInstLogType.HIDDEN_FORM, AippInstLogType.FILE)
    ...
    void shouldQueryDbWithTimeWhenCallQueryInstanceLogSinceWithTimeString() {
        ...
        // 截止时间后的记录
        List<AippInstLog> dummyAfterAippInstLog = Stream.of(AippInstLogType.MSG, AippInstLogType.FORM,
                        AippInstLogType.ERROR)
                .map(generateAippInstLogFunc)
                .peek(logData -> logData.setCreateAt(afterTimestamp))
                .collect(Collectors.toList());
        when(aippLogMapperMock.selectWithCondition(
                argThat(cond -> cond.getInstanceId().equals(DUMMY_ID) && cond.getAfterAt()
                        .equals(afterTimestamp)))).thenReturn(dummyAfterAippInstLog);
    ...
    void shouldSuccessWhenQueryAippRecentInstLog() {
        ...
        metaInstanceResult.setRange(new RangeResult(0, 1, 1));
        metaInstanceResult.setResults(Collections.singletonList(
                new Instance(DUMMY_ID, Collections.singletonMap(AippConst.INST_STATUS_KEY, dummyAippInstanceStatus),
                        null)));
        ...
        List<AippInstLog> dummyAippInstLog = Stream.of(AippInstLogType.MSG, AippInstLogType.FORM, AippInstLogType.ERROR)
                ...
                .collect(Collectors.toList());
        when(aippLogMapperMock.selectRecentByAippId(eq(DUMMY_ID), eq(DUMMY_AIPP_TYPE),
                eq(OperationContextDummy.DUMMY_W3_ACCOUNT))).thenReturn(dummyAippInstLog);
        when(metaInstanceServiceMock.list(eq(DUMMY_ID),
                argThat(filter -> filter.getIds().size() == 1 && filter.getIds().get(0).equals(DUMMY_ID)), eq(0L),
                eq(1), argThat(OperationContextDummy::operationContextDummyMatcher))).thenReturn(metaInstanceResult);

        List<AippInstLogDataDto> result = aippLogService.queryAippRecentInstLog(DUMMY_ID, DUMMY_AIPP_TYPE,
        ...
        List<Long> logIdSequence = logDto.getInstanceLogBodies()
                ...
                .collect(Collectors.toList());
        Assertions.assertIterableEquals(logIdSequence.stream().sorted().collect(Collectors.toList()), logIdSequence);
        verify(aippLogMapperMock, times(1)).selectRecentByAippId(eq(DUMMY_ID), eq(DUMMY_AIPP_TYPE),
    ...
    void shouldSortedWhenQueryAippRecentInstLog() {
        ...
        final List<String> dummyInstIdList = LongStream.rangeClosed(1, instanceCount)
        ...
        when(aippLogMapperMock.selectRecentByAippId(eq(DUMMY_ID), eq(DUMMY_AIPP_TYPE),
                eq(OperationContextDummy.DUMMY_W3_ACCOUNT))).thenReturn(dummyAippInstLog);
        RangedResultSet<Instance> metaInstanceResult = new RangedResultSet<>(Collections.singletonList(
                new Instance(null, Collections.singletonMap(AippConst.INST_STATUS_KEY, dummyAippInstanceStatus), null)),
                new RangeResult(0, 1, 1));
        when(metaInstanceServiceMock.list(eq(DUMMY_ID),
                argThat(filter -> filter.getIds().size() == 1 && dummyInstIdList.contains(filter.getIds().get(0))),
                eq(0L), eq(1), argThat(OperationContextDummy::operationContextDummyMatcher))).thenReturn(
                metaInstanceResult);
        this.mockMta();
    ...
    void shouldOkWhenCallQueryChatRecentChatLog() {
        ...
        when(this.metaServiceMock.list(any(MetaFilter.class), anyBoolean(), anyLong(), anyInt(),
                any(OperationContext.class), any(MetaFilter.class))).thenReturn(metaRangedResultSet);
        when(this.metaServiceMock.list(any(MetaFilter.class), anyBoolean(), anyLong(), anyInt(),
    ...
    void testQueryRecentLogsSinceResume() {
        ...
        when(this.aippLogService.queryBatchAndFilterFullLogsByLogType(instanceIds, any())).thenReturn(aippInstLogList);
        List<AippInstLogDataDto> list = this.aippLogService.queryRecentLogsSinceResume("1", "1",
                OperationContextDummy.getDummy());
        Assertions.assertEquals(list.get(0).getInstanceId(), "1");
        Assertions.assertEquals(list.get(1).getInstanceId(), "2");
        Assertions.assertEquals(list.get(2).getInstanceId(), "3");
    ...
    void testQueryAippRecentInstLogAfterSplice() {
        ...
        when(this.aippLogService.queryBatchAndFilterFullLogsByLogType(instanceIds, any())).thenReturn(aippInstLogList);
        List<AippInstLogDataDto> list = this.aippLogService.queryAippRecentInstLogAfterSplice("1", "1", 3,
                OperationContextDummy.getDummy());
        Assertions.assertEquals(list.get(0).getInstanceId(), "1");
        Assertions.assertEquals(list.get(1).getInstanceId(), "2");
        Assertions.assertEquals(list.get(2).getInstanceId(), "3");
    ...
    List<AippInstLog> generateAippInstLogList() {
        ...
        AippInstLog aippInstLog1 = AippInstLog.builder()
                .aippId("1")
                .logId(1L)
                .instanceId("1")
                .logType("MSG")
        ...
        AippInstLog aippInstLog2 = AippInstLog.builder()
                .aippId("1")
                .logId(2L)
                .instanceId("2")
                .logType("MSG")
                .path("/2")
                .createAt(LocalDateTime.now().plusMinutes(1))
                .build();
        aippInstLogList.add(aippInstLog2);
        AippInstLog aippInstLog3 = AippInstLog.builder()
                .aippId("1")
                .logId(3L)
                .instanceId("3")
                .logType("MSG")
                .path("/3")
                .createAt(LocalDateTime.now().plusMinutes(2))
                .build();
    ...
    void shouldOkWhenUpdateLog() {
        ...
        verify(this.aippLogMapperMock, times(1)).updateDataAndType(123L, "logType", "logData");
    }
}
