...
class AippFlowServiceTest {
    ...
    void testQueryAippDetailThenOk() {
        ...
        when(metaServiceMock.list(
                argThat(metaFilter -> metaFilter.getCategories().size() == 1 && metaFilter.getCategories()
                        .get(0)
                        .equals(JaneCategory.AIPP.name()) && metaFilter.getMetaIds().size() == 1
                        && metaFilter.getMetaIds().get(0).equals("testAippId") && metaFilter.getVersions().size() == 1
                        && metaFilter.getVersions().get(0).equals(defaultVersion)), eq(true), eq(0L), eq(1),
        ...
        when(flowsServiceMock.getFlows(eq((String) attributes.get(AippConst.ATTR_FLOW_CONFIG_ID_KEY)),
                eq((String) attributes.get(AippConst.ATTR_VERSION_KEY)), any(OperationContext.class))).thenReturn(
                flowInfo);

        Rsp<AippDetailDto> rsp = aippFlowServiceImpl.queryAippDetail(meta.getId(), defaultVersion,
    ...
    void testQueryAippDetailWithGetFlowsErrorThenFail() {
        ...
        RangedResultSet<Meta> mockResult = RangedResultSet.create(Collections.singletonList(meta), 0L, 1, 1);
        when(metaServiceMock.list(any(MetaFilter.class), eq(true), eq(0L), eq(1), any(OperationContext.class),
        ...
        doThrow(new JobberParamException(INPUT_PARAM_IS_EMPTY, "flowId")).when(flowsServiceMock)
                .getFlows(eq(ObjectUtils.cast(attributes.get(AippConst.ATTR_FLOW_CONFIG_ID_KEY))),
                        eq(ObjectUtils.cast(attributes.get(AippConst.ATTR_VERSION_KEY))), any(OperationContext.class));
    ...
    void shouldSetDraftVersionWhenCallListAippWithInactiveAipp() {
        ...
        when(metaServiceMock.list(any(MetaFilter.class), eq(true), eq(0L), eq(10),
                any(OperationContext.class))).thenReturn(
    ...
    void testUpdateAippWithInvalidConditionThenFail() {
        ...
        when(metaServiceMock.list(
                argThat(metaFilter -> metaFilter.getCategories().size() == 1 && metaFilter.getCategories()
                        .get(0)
                        .equals(JaneCategory.AIPP.name()) && metaFilter.getMetaIds().size() == 1
                        && metaFilter.getMetaIds().get(0).equals("testId") && metaFilter.getVersions().size() == 1
                        && metaFilter.getVersions().get(0).equals(DUMMY_META_VERSION_OLD)), eq(true), eq(0L), eq(1),
                any(OperationContext.class))).thenReturn(mockResult);
    ...
    void testUpdateAippThenOk() {
        ...
        when(metaServiceMock.list(
                argThat(metaFilter -> metaFilter.getCategories().size() == 1 && metaFilter.getCategories()
                        .get(0)
                        .equals(JaneCategory.AIPP.name()) && metaFilter.getMetaIds().size() == 1
                        && metaFilter.getMetaIds().get(0).equals("testId") && metaFilter.getVersions().isEmpty()),
                eq(true), eq(0L), eq(1), any(OperationContext.class))).thenReturn(mockResult);
        doAnswer(var -> {
    ...
    void testUpdateAippWithUpdateFlowsFailThenFail() {
        ...
        doThrow(new JobberException(FLOW_GRAPH_SAVE_ERROR, "1", "1.0.0")).when(flowsServiceMock)
                .updateFlows(eq(ObjectUtils.cast(expectMeta.getAttributes().get(AippConst.ATTR_FLOW_CONFIG_ID_KEY))),
                        eq(ObjectUtils.cast(expectMeta.getAttributes().get(AippConst.ATTR_VERSION_KEY))), any(),
                        any(OperationContext.class));
        Assertions.assertThrows(AippException.class, () -> {
    ...
    private void publishBasicMock(Meta expectMeta) {
        ...
        when(metaServiceMock.list(
                argThat(metaFilter -> metaFilter.getCategories().size() == 1 && metaFilter.getCategories()
                        ...
                        .equals(JaneCategory.AIPP.name()) && metaFilter.getMetaIds().size() == 1
                        && metaFilter.getMetaIds().get(0).equals("testId") && metaFilter.getVersions().isEmpty()),
                eq(true), eq(0L), eq(1), any(OperationContext.class), any(MetaFilter.class))).thenReturn(mockResult);
    ...
    void shouldOkWhenPreviewAipp() {
        ...
        AippCreateDto rsp = aippFlowServiceImpl.previewAipp(
                (String) expectMeta.getAttributes().get(AippConst.ATTR_VERSION_KEY), aipp, GenTestOperationContext());
    ...
    void shouldFailedWhenCreatePreviewAippFailed() {
        ...
        AippDto aipp = GenAippDtoWithData(expectMeta.getName());
        aipp.setId(expectMeta.getId());
        Assertions.assertThrows(AippException.class, () -> this.aippFlowServiceImpl.previewAipp(
                ObjectUtils.cast(expectMeta.getAttributes().get(AippConst.ATTR_VERSION_KEY)), aipp,
                GenTestOperationContext()));
    }
