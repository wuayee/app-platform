...
public class AippRunTimeServiceTest {
    ...
    void shouldOkWhenCreateInstance() {
        ...
        when(metaServiceMock.list(any(MetaFilter.class), eq(true), eq(0L), eq(1),
                any(OperationContext.class))).thenReturn(
                RangedResultSet.create(Collections.singletonList(expectMeta), 0L, 1, 1L));
        ...
        String instId = runTimeService.createAippInstance(DUMMY_AIPP_ID, DUMMY_AIPP_VERSION, initContext,
                genTestOpContext());
    ...
    void shouldFailedWhenDeleteInvalidAipp() {
        ...
        RangedResultSet<Instance> res = RangedResultSet.create(Collections.singletonList(new Instance()),
                new RangeResult(0, 0, 0));
        ...
        Assertions.assertThrows(JobberException.class,
                () -> runTimeService.deleteAippInstance(DUMMY_AIPP_ID, DUMMY_AIPP_VERSION, DUMMY_INST_ID,
                        genTestOpContext()));

        Instance inst = new Instance(DUMMY_INST_ID,
                Collections.singletonMap("invalid_key", MetaInstStatusEnum.ARCHIVED.name()), null);
    ...
    void shouldOkWhenResumeInstance() {
        ...
        Mockito.doNothing()
                ...
                .patchMetaInstance(any(), eq(DUMMY_INST_ID),
                        argThat(info -> info.getInfo().getValue().containsKey(DUMMY_PROPS_KEY)), any());
        ...
        Assertions.assertDoesNotThrow(
                () -> runTimeService.resumeAndUpdateAippInstance(DUMMY_INST_ID, formArgs, 1L, genTestOpContext(),
                        true));
    }
    ...
    void shouldOkWhenTerminateInstance() {
        ...
        Mockito.doNothing()
                .when(metaInstanceServiceMock)
                .patchMetaInstance(eq(DUMMY_META_VERSION_ID), eq(DUMMY_INST_ID),
                        argThat(info -> info.getInfo().getValue().containsKey(AippConst.INST_STATUS_KEY)), any());
    ...
    void shouldThrowExceptionWhenTerminateFlowsFailed() {
        ...
        Mockito.doThrow(JobberException.class).when(flowInstanceServiceMock).terminateFlows(any(), any(), any(), any());
    ...
    private static class MetaBuilder {
        ...
        public Meta build() {
            return new Meta(id, name, category, properties, creator, creationTime, lastModifier, lastModificationTime,
                    tenant, attributes, version, versionId);
        }
    }
}
