...
public class AippRunTimeServiceTest {
    ...
    @Test
    void shouldFailWhenQueryInvalidEdge() {
        Assertions.assertThrows(AippParamException.class,
                () -> runTimeService.queryEdgeSheetData(DUMMY_AIPP_ID, DUMMY_AIPP_VERSION, "invalid_edge", genTestOpContext()));
    }

    @Test
    void shouldFailWhenQueryEdgeWithInvalidStartFormInfo() {
        Meta meta = MetaBuilder.custom().build();
        TestUtils.mockMetaListReturnSingleItem5(meta, metaServiceMock);

        Assertions.assertThrows(AippNotFoundException.class,
                () -> runTimeService.queryEdgeSheetData(DUMMY_AIPP_ID, DUMMY_AIPP_VERSION, "start", genTestOpContext()));

        meta.setAttributes(Collections.singletonMap(AippConst.ATTR_START_FORM_ID_KEY, DUMMY_START_FORM_ID));
        TestUtils.mockMetaListReturnSingleItem5(meta, metaServiceMock);
    ...
    @Test
    void shouldOkWhenQueryEdgeWithNormalStartFormInfo() {
        Meta meta = MetaBuilder.custom()
                .putAttr(AippConst.ATTR_START_FORM_ID_KEY, DUMMY_START_FORM_ID)
                .putAttr(AippConst.ATTR_START_FORM_VERSION_KEY, DUMMY_START_FORM_VERSION)
                .build();
        TestUtils.mockMetaListReturnSingleItem5(meta, metaServiceMock);

        Mockito.doAnswer(var -> {
            String fromId = var.getArgument(0);
            String version = var.getArgument(1);
            Assertions.assertEquals(DUMMY_START_FORM_ID, fromId);
            Assertions.assertEquals(DUMMY_START_FORM_VERSION, version);
            return new DynamicFormDetailEntity(null, null);
        }).when(dynamicFormServiceMock).queryFormDetailByPrimaryKey(any(), any(), any());
    ...
    void shouldOkWhenCreateInstance() {
        ...
        verify(uploadedFileManageServiceMock, times(1)).addFileRecord(any(), any(), any());
    }
    ...
    void shouldOkWhenQueryAipp() {
        ...
        Mockito.doReturn(null)
                .when(dynamicFormServiceMock)
                .queryFormDetailByPrimaryKey(eq(DUMMY_CURR_FORM_ID), eq(DUMMY_CURR_FORM_VERSION_ID), any());

        AippInstanceDto rsp =
                runTimeService.getInstance(DUMMY_AIPP_ID, DUMMY_AIPP_VERSION, DUMMY_INST_ID, genTestOpContext());
        Assertions.assertEquals(DUMMY_INST_NAME, rsp.getAippInstanceName());
    }

    @Test
    void shouldOkWhenListAipp() {
        Meta meta = MetaBuilder.custom().name(DUMMY_AIPP_NAME).build();
        TestUtils.mockMetaListReturnSingleItem5(meta, metaServiceMock);
        RangedResultSet<Instance> res = genTestRangedResultSet(MetaInstStatusEnum.ARCHIVED.name());
        List<FormMetaQueryParameter> dummyParameters =
                Collections.singletonList(new FormMetaQueryParameter(DUMMY_CURR_FORM_ID, DUMMY_CURR_FORM_VERSION_ID));
        Mockito.doReturn(Collections.emptyList())
                .when(dynamicFormServiceMock)
                .queryFormDetailByPrimaryKeyAndMap(eq(dummyParameters), any());

        AippInstanceQueryCondition cond = AippInstanceQueryCondition.builder()
                .order(DirectionEnum.ASCEND.name())
                .sort(MetaInstSortKeyEnum.END_TIME.name())
                .creator(DUMMY_CREATOR)
                .aippInstanceName(DUMMY_INST_NAME)
                .build();
        PaginationCondition page = PaginationCondition.builder().pageNum(1).pageSize(5).build();

        Mockito.doReturn(res)
                .when(metaInstanceServiceMock)
                .list(any(),
                        argThat(filter -> filter.getInfos().containsKey(AippConst.INST_CREATOR_KEY)),
                        eq(page.getOffset()),
                        eq(page.getPageSize()),
                        any());

        PageResponse<AippInstanceDto> rsp =
                runTimeService.listInstance(DUMMY_AIPP_ID, DUMMY_AIPP_VERSION, cond, page, genTestOpContext());
        Assertions.assertEquals(1, rsp.getTotal());
        Assertions.assertEquals(DUMMY_AIPP_NAME, rsp.getLabel());
        Assertions.assertEquals(1, rsp.getItems().size());
    }
