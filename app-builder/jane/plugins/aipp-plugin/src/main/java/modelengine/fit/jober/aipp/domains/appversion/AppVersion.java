...
public class AppVersion {
    ...
    private void startTask(RunContext context, ChatSession<Object> session) {
        ...
        context.putAllToBusiness(context.getUserContext());
    ...
    private void updateFlows(OperationContext context) {
        if (this.isUpdated()) {
    ...
    private List<AppTask> getPublishedTasks(OperationContext ctx) {
     ...
     */
    public void restart(String instanceId, Map<String, Object> restartParams, ChatSession<Object> session,
    ...
    public AppExportDto export(OperationContext context, Map<String, String> exportMeta) {
        ...
        try {
            String flowDefinitionData = this.aippFlowDefinitionService.getParsedGraphData(
                    JsonUtils.toJsonString(this.getFlowGraph().getAppearance()), this.getData().getVersion());
    ...
    public void cloneVersion(AppVersion cloneApp) {
        ...
        AppBuilderFlowGraph currentGraph = cloneApp.getFlowGraph();
        String currentGraphId = cloneApp.getFlowGraph().getId();
        resetGraph.setId(currentGraphId);
        resetGraph.resetGraphId();

        currentGraph.setAppearance(JSONObject.toJSONString(resetGraph.getAppearance()));
