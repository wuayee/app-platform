...
public class AppBuilderAppServiceImpl
    ...
    public AppBuilderAppDto query(String appId, OperationContext context) {
        return this.appVersionService.getByAppId(appId)
    ...
    public Rsp<RangedResultSet<AppBuilderAppMetadataDto>> list(AppQueryCondition cond,
        ...
        RangedResultSet<AppVersion> result = this.appVersionService.pageListByTenantId(cond, tenantId, offset, limit);
    ...
    public List<PublishedAppResDto> recentPublished(AppQueryCondition cond, long offset, int limit, String appId,
        ...
        List<AppTask> publishedTasks = this.appTaskService.getTaskList(aippId, NORMAL.name(), ACTIVE.getCode(), context)
                .stream()
                .filter(t -> !t.isBelongApp(appId))
                .toList();
        return publishedTasks.stream().map(t -> AppTaskUtils.toPublishedAppResDto(t, appVersion)).toList();
    }
