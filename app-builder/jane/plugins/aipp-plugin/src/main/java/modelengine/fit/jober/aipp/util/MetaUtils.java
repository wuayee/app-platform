...
public class MetaUtils {
    ...
    private static Meta getSingleMetaHandle(MetaService metaService, String metaId, MetaFilter metaFilter,
        ...
        RangedResultSet<Meta> metaRes = metaService.list(metaFilter, true, 0, 1, context,
                buildOldDataMetaFilter(metaFilter));
        if (metaRes.getResults().isEmpty()) {
            log.warn("meta {} version {} meta status {} not found", metaId, metaFilter.getVersions(),
    ...
    public static List<Meta> getListMetaHandle(MetaService metaService, MetaFilter metaFilter, OperationContext context)
        ...
        return getAllFromRangedResult(limitPerQuery,
                (offset) -> metaService.list(metaFilter, false, offset, limitPerQuery, context,
    ...
    public static MetaFilter buildOldDataMetaFilter(MetaFilter metaFilter) {
        MetaFilter oldDataFilter = new MetaFilter(metaFilter.getMetaIds(), metaFilter.getVersionIds(),
    ...
    public static boolean isPublished(Meta meta) {
        ...
        if (!attributes.containsKey(AippConst.ATTR_AIPP_TYPE_KEY) || !attributes.containsKey(
                AippConst.ATTR_META_STATUS_KEY)) {
            return false;
        }
