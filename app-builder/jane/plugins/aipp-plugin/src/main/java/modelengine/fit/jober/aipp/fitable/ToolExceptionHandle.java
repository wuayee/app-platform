...
public class ToolExceptionHandle {
     ...
     */
    public ToolExceptionHandle(@Fit LocaleService localeService,
        @Fit PluginToolService pluginToolService) {
    ...
    private Optional<String> getToolName(FlowErrorInfo errorMessage) {
        ...
        if (properties != null && properties.containsKey(TOOL_ID)
            && StringUtils.isNotBlank(properties.get(TOOL_ID))) {
    ...
    private String getErrorMessageTemplate(Boolean isDialogue, Locale locale, boolean hasToolName) {
        AippErrCode errCode = isDialogue ? (hasToolName ? AippErrCode.DIALOGUE_TOOL_NODE_ERROR_OUTPUT
            : AippErrCode.DIALOGUE_GENERAL_NODE_ERROR_OUTPUT)
            : (hasToolName ? AippErrCode.ELSA_TOOL_NODE_ERROR_OUTPUT
                : AippErrCode.ELSA_GENERAL_NODE_ERROR_OUTPUT);
    ...
    private static boolean isGeneralError(int errorCode) {
        return isInRange(errorCode, 0x7F010000, 0x7F01FFFF)
            || isInRange(errorCode, 0x7F050000, 0x7F05FFFF)
            || isInRange(errorCode, 0x7F060000, 0x7F06FFFF)
            || isInRange(errorCode, 0x7F100000, 0x7F1FFFFF)
            || isInRange(errorCode, 0x7F200000, 0x7F2FFFFF)
            || isInRange(errorCode, 0x7FF00000, 0x7FFFFFFF);
    }

    private static boolean isRoutingOrLoadBalancingError(int errorCode) {
        return isInRange(errorCode, 0x7F020000, 0x7F02FFFF)
            || isInRange(errorCode, 0x7F030000, 0x7F03FFFF);
    }
