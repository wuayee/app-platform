...
public class LlmComponent implements FlowableService, FlowCallbackService, FlowExceptionService {
    ...
    public void callback(List<Map<String, Object>> childFlowData) {
        ...
        StreamMsgSender streamMsgSender = new StreamMsgSender(this.aippLogStreamService, this.serializer, path, msgId,
                parentInstanceId);
        agentFlow.converse()
    ...
    private boolean checkModelAvailable(Map<String, Object> businessData) {
        ...
        return this.aippModelCenter.fetchModelList(AippConst.CHAT_MODEL_TYPE, DataUtils.getOpContext(businessData))
                .getModels()
                .stream()
    ...
    private void addAnswer(AippLlmMeta llmMeta, String answer, Map<String, Object> promptMetadata) {
        ...
        this.metaInstanceService.patchMetaInstance(llmMeta.getVersionId(), llmMeta.getInstId(), info,
                llmMeta.getContext());
        doOnAgentComplete(llmMeta);
    }
    ...
    private void setChildInstanceId(AippLlmMeta llmMeta, String childInstanceId) {
        InstanceDeclarationInfo info = InstanceDeclarationInfo.custom()
                .putInfo(AippConst.INST_CHILD_INSTANCE_ID, childInstanceId)
                .build();
        this.metaInstanceService.patchMetaInstance(llmMeta.getVersionId(), llmMeta.getInstId(), info,
                llmMeta.getContext());
    }
    ...
    private void doOnAgentComplete(AippLlmMeta llmMeta) {
        ...
        // resumeFlow
        this.flowInstanceService.resumeAsyncJob(llmMeta.getFlowDataId(), llmMeta.getBusinessData(),
                llmMeta.getContext());
    }
    ...
    private void failLlmComponentNode(final AippLlmMeta llmMeta, final JoberErrorInfo joberErrorInfo) {
        // 修复失败场景未释放cache的问题
        llmCache.remove(llmMeta.getInstId());
        this.flowInstanceService.failAsyncJob(llmMeta.getFlowDataId(), joberErrorInfo,
    ...
    private static class ModelExtraBody {
        ...
        public ModelExtraBody(String sessionId) {
            ...
            this.sessionId = sessionId.substring(0, 8) + "-" + sessionId.substring(8, 12) + "-" + sessionId.substring(
                    12, 16) + "-" + sessionId.substring(16, 20) + "-" + sessionId.substring(20);
        }
    }
    ...
    static class StreamMsgSender {
        ...
        public void sendMsg(String msg, Map<String, Object> businessData) {
            ...
            if (!enableLog || StringUtils.isBlank(msg)) {
                return;
            }
