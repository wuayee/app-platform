...
public class MetaMultiVersionFitable implements MetaService {
    ...
    private final modelengine.fit.jane.task.domain.TaskProperty.Repo taskPropertyRepo;
    ...
    public MetaMultiVersionFitable(TaskService taskService,
            modelengine.fit.jane.task.domain.TaskProperty.Repo taskPropertyRepo, MetaConverter metaConverter,
    ...
    public Meta create(MetaDeclarationInfo declaration, OperationContext context) {
        ...
        modelengine.fit.jane.task.util.OperationContext actualContext =
    ...
    private String queryTemplateIdOrCreateTemplate(OperationContext context, String name,
            Undefinable<String> basicMetaTemplateId, modelengine.fit.jane.task.util.OperationContext actualContext) {
        TaskTemplateFilter filter = new TaskTemplateFilter();
        filter.setNames(UndefinableValue.defined(Collections.singletonList("eq(" + name + ")")));
        modelengine.fitframework.model.RangedResultSet<TaskTemplate> list =
                this.taskTemplateRepo.list(filter, 0, 10, actualContext);
        if (list.getRange().getTotal() == 1) {
            return list.getResults().get(0).id();
        }
