...
import static modelengine.fit.jane.Undefinables.whenDefined;
...
public class MetaInstanceConverterImpl implements MetaInstanceConverter {
     ...
     */
    @Override
    public modelengine.fit.jane.meta.instance.Instance convert1(TaskEntity task, TaskInstance instance) {
        modelengine.fit.jane.meta.instance.Instance result = new modelengine.fit.jane.meta.instance.Instance();
        result.setInfo(instance.info().entrySet().stream()
                .collect(Collectors.toMap(Map.Entry::getKey, entry -> task.getProperties()
                        .stream()
                        .filter(property -> StringUtils.equals(property.name(), entry.getKey()))
     ...
     */
    @Override
    public TaskInstance.Filter convert(modelengine.fit.jane.meta.instance.MetaInstanceFilter metaInstanceFilter) {
        return TaskInstance.Filter.custom()
