...
import static com.huawei.fit.jober.flowsengine.domain.flows.enums.FlowJoberProperties.ECHO_PREFIX;
import static com.huawei.fit.jober.flowsengine.domain.flows.enums.FlowJoberProperties.SKIP_VARIABLES;
...
public class FlowEchoJober extends FlowJober {
    ...
    protected List<FlowData> executeJober(List<FlowData> inputs) {
        String echoSign = this.properties.getOrDefault(ECHO_PREFIX.getValue(), DEFAULT_ECHO_PREFIX);
        String skipVariableRaw = this.properties.getOrDefault(SKIP_VARIABLES.getValue(), StringUtils.EMPTY);
