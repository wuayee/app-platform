...
import static modelengine.fit.waterflow.common.Constant.CREATED_DATE;
import static modelengine.fit.waterflow.common.Constant.PRIORITY;
import static modelengine.fit.waterflow.common.Constant.REQUIREMENT_ID;
import static modelengine.fit.waterflow.common.Constant.STATUS;
import static modelengine.fit.waterflow.flowsengine.utils.FlowUtil.replace;
        ...
        Map<String, Object> propertyMap = new HashMap<String, Object>() {
            {
                ...
                put(PRIORITY, "ordinary");
                put(REQUIREMENT_ID, flowContextId);
                put(CREATED_DATE, getTime());
            }
        };

        propertyList.stream()
                .filter(taskProperty -> !Objects.equals(taskProperty.getName(), STATUS))
                .forEach(taskProperty -> {
                    String key = taskProperty.getName();
                    Object instancePropertyValue = inputProperties.containsKey(key) ? replace(inputProperties.get(key),
                            businessData) : businessData.get(key);
