 ...
 * Description: DataBus Python SDK的`databus.memory`模块实现
...
// databus.memory.read函数注释文档
static const char * const DATABUS_MEMORY_READ_DOC = \
 ...
 */
static PyObject*
DataBus_Memory_Read(PyObject* self, PyObject* args, PyObject* keywords)
{
    ...
    if (sharedMemoryBuffer == (void*) -1) {
        ...
        return PyErr_Format(PyExc_IOError, "fail to attach the shared memory");
    }
...
// 声明databus.memory模块的所有方法
static PyMethodDef g_dataBus_Memory_Methods[] = {
    {
        "read",
        // Python方法声明只接受两个参数的函数,需要强转一下
        (PyCFunction)(void(*)(void))DataBus_Memory_Read,
        METH_VARARGS | METH_KEYWORDS,
        DATABUS_MEMORY_READ_DOC
    },
    {NULL, NULL, 0, NULL}
};

// 声明databus.memory模块
static struct PyModuleDef g_dataBus_Memory_Module = {
    ...
    -1,
    // 模块内方法
    g_dataBus_Memory_Methods
};

// 初始化databus.memory模块
#define DATABUS_DECL_MODULE(moduleName) PyInit_##moduleName(void)

PyMODINIT_FUNC DATABUS_DECL_MODULE(memory)
{
    return PyModule_Create(&g_dataBus_Memory_Module);
}
