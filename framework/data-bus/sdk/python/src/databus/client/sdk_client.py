...
from databus.message import CoreErrorType
...
class SdkClient:
    ...
    def open(self, core_host: str, core_port: int) -> CoreErrorType:
        """关闭现有连接, 连接到新地址(core_host, core_port), 注意会**重置共享内存信息**

        :param core_host: DataBus内核地址
        :param core_port: DataBus内核监听端口
        :return 若平台不支持则返回CoreErrorType.PlatformNotSupported, 否则返回CoreErrorType.None_
        """
        if not core_host:
        ...
        if platform.system().lower() not in self.SUPPORTED_PLATFORMS:
            return CoreErrorType.PlatformNotSupported

        self._impl.open(core_host=core_host, core_port=core_port)
        return CoreErrorType.None_
    ...
    def shared_malloc(self, user_key: str, size: int) -> CoreErrorType:
        ...
        try:
            ...
            return CoreErrorType.NotConnectedToDataBus
        return CoreErrorType.None_

    def shared_free(self, user_key: str) -> CoreErrorType:
    ...
    def read_once(self, user_key: str, size: Optional[int] = None, offset: int = 0) \
            -> Tuple[CoreErrorType, Optional[bytes]]:
        ...
        """
        try:
            return CoreErrorType.None_, self._impl.read_once(user_key, size=size, offset=offset)
        except CoreError as e:
            return e.error_code, None
        except NotConnectedError:
            return CoreErrorType.NotConnectedToDataBus, None
        except IOError:
            return CoreErrorType.MemoryReadError, None

    def write_once(self, user_key: str, contents: bytes, offset: int = 0) -> CoreErrorType:
        ...
        try:
            ...
            if not write_len:
                return CoreErrorType.IOOutOfBounds
            return CoreErrorType.None_
        except CoreError as e:
            return e.error_code
        except NotConnectedError:
            ...
            return CoreErrorType.MemoryWriteError
