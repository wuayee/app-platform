...
import com.huawei.fit.waterflow.flowsengine.domain.flows.streams.FitStream.Source;
import com.huawei.fit.waterflow.flowsengine.domain.flows.streams.Processors.ArrayInvoke;
import com.huawei.fit.waterflow.flowsengine.domain.flows.streams.Processors.Invoke;
...
public class DataProducer<T> extends IdGenerator implements Source<T> {
     ...
     */
    @Override
    public Publisher<T> mono(Invoke<T> invoker) {
        this.from = this.from();
     ...
     */
    @Override
    public Publisher<T> flux(ArrayInvoke<T> invoker) {
        this.from = this.from();
