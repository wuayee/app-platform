export let uuid = function (isLong) {
    if (isLong) {
 ...
 */
export let getRotatedCoordinate = (x, y, cx, cy, degree) => {
    let dx = x - cx, dy = y - cy;
    return {
...
export let convertPositionWithParents = (parent, x, y) => {
    let parents = [];
    let rotateDegree = 0;
    while (parent !== parent.page) {
        rotateDegree += parent.rotateDegree;
        parents.push(parent);
    ...
    let convertPositionWithParent = (parents, x, y) => {
        ...
        if (parent.scaleX !== undefined) {
            x = parent.x + (x - parent.x) / parent.scaleX;
        }
        if (parent.scaleY !== undefined) {
            y = parent.y + (y - parent.y) / parent.scaleY;
        }
        let degree = parent.rotateDegree * Math.PI / 180;
        let cx = parent.x + parent.width / 2, cy = parent.y + parent.height / 2;
        let p = getRotatedCoordinate(x, y, cx, cy, -degree);
        parents.pop();
...
export let getInteractRect = (rect1, rect2) => {
...
export let getPixelRatio = (context) => {
    if (!context) {
...
export let pixelRateAdapter = function (context, pageScaleX, pageScaleY, ignorePageScale = false, ignoreHighQuality = false) {
    let canvas = context.canvas;
    const pixelRatio = getPixelRatio(context);

    let ratioX = ignoreHighQuality ? 1 : pixelRatio, ratioY = ignoreHighQuality ? 1 : pixelRatio;
    if (!ignorePageScale) {
...
export let isPointInRect = function (point, rect) {
    try {
...
export let isRectInRect = function (rect1, rect2) {
...
export let getDistance = function (x1, y1, x2, y2) {
    return Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
};

export let closestPoint = function (points1, points2) {
...
//画布相对浏览器的原始偏移（与缩放比无关）
export let offsetPosition = function (page) {
    // 全屏状态
    ...
    let top = page.getOffsetTop(), left = page.getOffsetLeft();
...
export let position = function (canvas) {
    let offset = function () {
        let top = 0, left = 0;
        let element = canvas;
        do {
...
export let exceptionIgnore = function (f, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10) {
    try {
...
export let isRectInteractRect = function (rect1, rect2) {
...
export let getGridGrabPosition = function (point, graphPage) {
...
export let sleep = async sleepDuration => {
    await new Promise(r => setTimeout(r, sleepDuration));
};

export let requestAnimationFrame = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame;

export let colorRgb = function (colorName) {
...
export const showDialog = (stirng, type) => {
    alert(string);
};
