...
export const domtoimage = (function (global) {
  ...
  function toSvg(node, options) {
    const optionsVal = options || {};
    copyOptions(optionsVal);
    return Promise.resolve(node)
      .then(function (node) {
        return cloneNode(node, optionsVal.filter, true);
      })
      .then(node => {
      ...
      .then(applyOptions)
      .then(function (clone) {
        return makeSvgDataUri(clone, optionsVal.width || util.width(node), optionsVal.height || util.height(node));
      });

    function applyOptions(clone) {
      if (optionsVal.bgcolor) {
        clone.style.backgroundColor = optionsVal.bgcolor;
      }

      if (optionsVal.width) {
        clone.style.width = `${optionsVal.width}px`;
      }
      if (optionsVal.height) {
        clone.style.height = `${optionsVal.height}px`;
      }

      if (optionsVal.style) {
        Object.keys(optionsVal.style).forEach((property) => {
          clone.style[property] = optionsVal.style[property];
        });
      }
  ...
  function toJpeg(node, options) {
    const optionsVal = options || {};
    return draw(node, optionsVal)
      .then(function (canvas) {
        return canvas.toDataURL('image/jpeg', optionsVal.quality || 1.0);
      });
  }
  ...
  function newUtil() {
    ...
    function getAndEncode(url) {
      ...
      if (domtoimage.impl.options.cacheBust) {
        ...
        urlVal += ((/\?/).test(urlVal) ? '&' : '?') + (new Date()).getTime();
      }
      ...
      return new Promise(function (resolve) {
        ...
        function done() {
          ...
          if (request.status !== 200) {
            if (placeholder) {
              ...
              fail(`cannot fetch resource: ${urlVal}, status: ${request.status}`);
            }
        ...
        function timeout() {
          if (placeholder) {
            ...
            fail(`timeout of ${TIMEOUT}ms occured while fetching resource: ${urlVal}`);
          }
        }
