...
const guideLineUtil = (page) => {
    ...
    self.init = () => {
        ...
        compareShapes = page.shapes.filter(s => {
    ...
    self._compare = (current, compare, position) => {
        ...
        directions.x.map(dire => {
            ...
            if (// 当前操作图形接近参照图形，且已进入吸附范围
                (near && isNearDirection) || // 当前操作图形离开参照图形，但仍在吸附范围，且离开步长小于1.1(1.1是试出来的 没有科学依据)
                (near && !isNearDirection && Math.abs(position.deltaX) < 0.8)) {
        ...
        directions.y.map(dire => {
            ...
            if (// 当前操作图形接近参照图形，且已进入吸附范围
                (near && isNearDirection) || // 当前操作图形离开参照图形，但仍在吸附范围，且离开步长小于1.1(1.1是试出来的 没有科学依据)
                (near && !isNearDirection && Math.abs(position.deltaY) < 0.8)) {
    ...
    self._adsorb = (current, position, key, results) => {
        ...
        if (resultsArray.length) {
            ...
            // TODO：当多个图形在相同坐标处对齐时，会出现重复线条，待去除
            // 去除逻辑，当value相同时，origin相同，length取最大值，origin不同时 length求和
            // let linesGroup = activeLines.reduce(function (pre, current, index) {
            //     pre[current.value] = pre[current.value] || []
            //     pre[current.value].push(current)
            //     return pre
            // }, {});
            // for (let attr in linesGroup) {
            //     let group = linesGroup[attr];
            //     if (group.length > 1) {
            //         group.map(l => {
            //
            //         })
            //     }
            //     console.log(attr, linesGroup[attr])
            // }
