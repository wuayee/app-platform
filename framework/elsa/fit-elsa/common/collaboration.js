...
const collaboration = (graph, url) => {
    ...
    self.communicator = ((session, url) => {
                ...
                self.invoke({method: "ping"}).then(users=>{
                    ...
                    graph.pages.forEach(p=>{
                        ...
                        if(graph.activePage && p.id===graph.activePage.id){
                            graph.activePage.shapes.forEach(s=>{
                                s.editBy = shapes[s.id];
                            })
                        }
                    })
                })
                //todo:use websocket ping;
            }
        };
        ...
        function openWebsocket() {
            ...
            console.log("webSocket init.", self.graph.collaborationSession);
        ...
        com.close = () => {
            ...
            console.log("webSocket close.", session);
        };
    ...
    self.invoke = async (args, callback) => {
        ...
        if (args.page) {
            // if (graph.activePage && graph.activePage.id===args.page && graph.activePage.isCoEditing === true) return;
            //handle local
            //造一个message，本地更新
            const message = {
        ...
        args.from = graph.cookie;// graph.session.id;
