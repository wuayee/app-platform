...
let drawer = (shape, div, x = 0, y = 0) => {
    ...
    // self.text.id = TEXT_ID;//"textArea:" + shape.id;
    ...
    self.transform = () => {
        ...
        compareAndSet(self.parent.style, 'transform', transformNew, () => {
            let transform = self.parent.style.transform = transformNew;
    ...
    // self.move = async () => {//react to shape.x,shape.y
    self.move = () => {//react to shape.x,shape.y
        let deltaX = shape.x, deltaY = shape.y;
        let finalX, finalY;
        if (shape.width < 0) {
            deltaX += shape.width;
        }
    ...
    self.parentResize = (width, height) => {
        ...
        self.refreshIndex();
        // self.parent.id = PARENT_ID; //shape.type + ":" + shape.id;
        // self.parent.name = PARENT_ID;
    ...
    self.updateIfChange = (obj, key, value, perfKey) => {
        ...
        if (self.perf[perfKey] === value) {
            return false;
        }
        self.perf[perfKey] = value;
        set(obj, key, value);
        return true;
    }
    ...
    self.updateCanvas = (width, height, canvasName) => {
        ...
        width = width + 1 * borderWidth + 2 * margin;
        height = height + 1 * borderWidth + 2 * margin;
        if (self.perf[canvasName + '_width'] !== width || self.perf[canvasName + '_height'] !== height || self.perf[canvasName + '_scaleX'] !== shape.page.scaleX || self.perf[canvasName + '_scaleY'] !== shape.page.scaleY) {
            self.perf[canvasName + '_width'] = width;
            self.perf[canvasName + '_height'] = height;
            self.perf[canvasName + '_scaleX'] = shape.page.scaleX;
            self.perf[canvasName + '_scaleY'] = shape.page.scaleY;
            self[canvasName].width = width;
            self[canvasName].height = height;
    ...
    self.textResize = () => {
        ...
        if (!shape.autoWidth) {
            // self.updateIfChange(self.text.style, 'maxWidth', Math.abs(width) + "px", 'text_max_width');
    ...
    self.drawRegions = () => {
        ...
        regions.forEach(r => {
            if (!r.disableCanvas) {
                ...
                canvas.style.zIndex = shape.index + 1 + (r.index ? r.index : 1);
    ...
    self.draw = function () {
        ...
        return () => {
            ...
            // todo@zhangyue 先注释掉，打开会导致缩略图无法显示
            // shape.page.actions.push(()=>{
            self.resize();
            // shape.indexCoordinate();
            self.transform();
...
let simpleDrawer = (shape, div, x, y) => {
    ...
    self.resize = () => {//react to shape.width,shape.height
        ...
        if (self.parent.style.visibility === "visible") {
            ...
            self.parent.style.zIndex = shpae.isFocused ? (shape.page.shapes.length + Z_INDEX_OFFSET) : shape.index;//getIndex();
...
let customizedDrawer = (elementName, preConstruct) => {
    return (shape, div, x, y) => {
        ...
        self.resize = () => {
            ...
            // self.element.style.width = self.parent.clientWidth - 2 * self.offset + "px";
            // self.element.style.height = self.parent.clientHeight - 2 * self.offset + "px";
