...
const rectangle = (id, x, y, width, height, parent, drawer = rectangleDrawer) => {
    ...
    // self.progressPercent = 0.75;//0-1
    // self.showedProgress = false;

    //-------------consts---------------
    let STEP = 13, OFFSET_Y = -7;
    ...
    self.dragTo = (position) => {
        ...
        const x = position.x, y = position.y, deltaX = position.deltaX, deltaY = position.deltaY;
    ...
    self.getMenuScript = () => {
        ...
        menus.push({
            text: "评论", action: (shape, x, y) => {
                let comment = shapeComment(shape.x + shape.width / 2, shape.y + shape.height / 2, shape);
                comment.invalidate();
                comment.beginEdit();
            }, draw: (context) => {
        ...
        menus.push({
            text: (self.emphasized ? "取消" : "") + "关注", action: shape => shape.emphasized = !shape.emphasized, draw: (context) => {
                context.strokeStyle = "dimgray";
        ...
        priorities.menus.push({ text: "无", action: shape => shape.priority = 0 });
        ...
        warns.menus.push({
            text: "提示", action: shape => shape.infoType = INFO_TYPE.INFORMATION, draw: (context) => {
                context.fillStyle = "steelblue";
        ...
        warns.menus.push({
            text: "警告", action: shape => shape.infoType = INFO_TYPE.WARNING, draw: (context) => {
        ...
        warns.menus.push({
            text: "错误", action: shape => shape.infoType = INFO_TYPE.ERROR, draw: (context) => {
                context.fillStyle = "red";
                context.font = "bold 11px Arial";
        ...
        warns.menus.push({ text: "无", action: shape => shape.infoType = INFO_TYPE.NONE });
        menus.push(warns);
        //状态
        const status = {
        ...
        status.menus.push({
            text: "未开始", action: shape => shape.progressStatus = PROGRESS_STATUS.NOTSTARTED, draw: (context) => {
                context.arc(0, 0, 4, 0, 2 * Math.PI);
                context.stroke();
                context.beginPath();
                context.fillStyle = "red";
        ...
        status.menus.push({
            text: "进行中", action: shape => shape.progressStatus = PROGRESS_STATUS.DOING, draw: (context) => {
                const STEPS = 10;
        ...
        status.menus.push({
            text: "运行中", action: shape => shape.progressStatus = PROGRESS_STATUS.RUNNING, draw: (context) => {
                context.fillStyle = "steelblue";
                context.moveTo(-3, -3);
                context.lineTo(3, 0);
                context.lineTo(-3, 3);
        ...
        status.menus.push({
            text: "暂停", action: shape => shape.progressStatus = PROGRESS_STATUS.PAUSE, draw: (context) => {
                context.fillStyle = "red";
        ...
        status.menus.push({
            text: "完成", action: shape => shape.progressStatus = PROGRESS_STATUS.COMPLETE, draw: (context) => {
                const x0 = -3, y0 = 1;
        ...
        status.menus.push({
            text: "错误", action: shape => shape.progressStatus = PROGRESS_STATUS.ERROR, draw: (context) => {
        ...
        status.menus.push({
            text: "未知", action: shape => shape.progressStatus = PROGRESS_STATUS.UNKNOWN, draw: (context) => {
                context.fillStyle = "dimgray";
                context.font = "bold 11px Arial";
                context.fillText("?", -3, 4);
        ...
        status.menus.push({ text: "无", action: shape => shape.progressStatus = PROGRESS_STATUS.NONE });
        menus.push(status);
        //进度
        const progress = {
        ...
        progress.menus.push({
            text: "15%", action: shape => setProgress(shape, 0.15), draw: (context) => {
        ...
        progress.menus.push({
            text: "30%", action: shape => setProgress(shape, 0.3), draw: (context) => {
        ...
        progress.menus.push({
            text: "45%", action: shape => setProgress(shape, 0.45), draw: (context) => {
        ...
        progress.menus.push({
            text: "60%", action: shape => setProgress(shape, 0.6), draw: (context) => {
        ...
        progress.menus.push({
            text: "75%", action: shape => setProgress(shape, 0.75), draw: (context) => {
        ...
        progress.menus.push({
            text: "90%", action: shape => setProgress(shape, 0.90)
            , draw: (context) => {
        ...
        progress.menus.push({ text: "无", action: shape => shape.showedProgress = false });
        menus.push(progress);
    ...
    self.initConnectors = () => {
        ...
        let visible = shape => {
            return shape.moveable;
        };
        //west,index:0
        let c0 = connector(self, (s, c) => c.frame.x - s.x - s.focusMargin, (s, c) => c.frame.y - s.y + c.frame.height / 2, s => DIRECTION.W, s => s.visible && visible(s) && self.getFrame().height >= CON_MIN, s => s.resizeable, s => true, release, s => false);
        let c0moving = (self, deltaX, deltaY, x, y) => {
            let degree = self.rotateDegree * Math.PI / 180;
            let dx = transformDelta(self, x, y, deltaX, deltaY).dx, dy = 0;
        ...
        //east,index:1
        let c1 = connector(self, (s, c) => c.frame.x - s.x + c.frame.width + s.focusMargin, (s, c) => c.frame.y - s.y + c.frame.height / 2, s => DIRECTION.E, s => s.visible && visible(s) && self.getFrame().height >= CON_MIN, s => s.resizeable, s => true, release, s => false);
        ...
        //north,index:2
        let c2 = connector(self, (s, c) => c.frame.x - s.x + c.frame.width / 2, (s, c) => c.frame.y - s.y - s.focusMargin, s => DIRECTION.N, s => s.visible && visible(s) && self.getFrame().width >= CON_MIN, s => s.resizeable, s => true, release, s => false);
        let c2moving = (self, deltaX, deltaY, x, y) => {
            let degree = self.rotateDegree * Math.PI / 180;
            let dx = 0, dy = transformDelta(self, x, y, deltaX, deltaY).dy;
            let offset = getRotatedCoordinate(dx, dy, 0, 0, degree);
        ...
        //south,index:3
        let c3 = connector(self, (s, c) => c.frame.x - s.x + c.frame.width / 2, (s, c) => c.frame.y - s.y + c.frame.height + s.focusMargin, s => DIRECTION.S, s => s.visible && visible(s) && self.getFrame().width >= CON_MIN, s => s.resizeable, s => true, release, s => false);
        ...
        let getC4Delta = (deltaX, deltaY) => {
            if (Math.abs(deltaX) > Math.abs(deltaY)) {
                deltaY = deltaX / proportion;
            } else {
                deltaX = deltaY * proportion;
            }
            return { deltaX, deltaY };
        }
        let c4 = connector(self, (s, c) => c.frame.x - s.x - s.focusMargin, (s, c) => c.frame.y - s.y - s.focusMargin, s => DIRECTION.NW, s => s.visible && visible(s), s => s.resizeable, s => false, release, s => self.page.shiftKeyPressed, getC4Delta);
        ...
        let c5 = connector(self, (s, c) => c.frame.x - s.x - s.focusMargin, (s, c) => c.frame.y - s.y + c.frame.height + s.focusMargin, s => DIRECTION.SW, s => s.visible && visible(s), s => s.resizeable, s => false, release, s => self.page.shiftKeyPressed, getC5Delta);
        ...
        let c6 = connector(self, (s, c) => c.frame.x - s.x + c.frame.width + s.focusMargin, (s, c) => c.frame.y - s.y - s.focusMargin, s => DIRECTION.NE, s => s.visible && visible(s), s => s.resizeable, s => false, release, s => self.page.shiftKeyPressed, getC6Delta);
        ...
        let c7 = connector(self, (s, c) => c.frame.x - s.x + c.frame.width + s.focusMargin, (s, c) => c.frame.y - s.y + c.frame.height + s.focusMargin, s => DIRECTION.SE, s => s.visible && visible(s), s => s.resizeable, s => false, release, s => self.page.shiftKeyPressed, getC7Delta);
        ...
        c7.type = "rightBottom";
        //??
        let c8 = connector(self, (s, c) => c.frame.x - s.x + c.frame.width / 2, (s, c) => c.frame.y - s.y - 13 - s.focusMargin, s => DIRECTION.R, s => s.visible && visible(s) && s.getRotateAble() && self.getFrame().width >= CON_MIN, s => s.getRotateAble(), s => false, release, s => false);
        //c8.radius = 5;
        c8.moving = (deltaX, deltaY, x, y) => {
            //todo:写不动了，这里需要转换坐标，在container已经旋转的情况下，可以正确旋转
    ...
    self.createUndragableConnector = ({rx, ry, key, ax, vector, direction}) => {
        ...
        }, s => s.visible, s => s.resizeable, () => true, () => {}, s => false);
        c.type = "connection";
        c.dragable = false;
        return c;
    };
    ...
    self.keyPressed = e => {
        ...
        if (isCtrl && e.code.indexOf("Digit") >= 0) {
            //self.priority = parseInt(e.code.substr(5, 1));
        ...
        if (e.shiftKey && e.code === "Digit2") {
            ...
            self.assign("544938", "huizi");//remove it after user interface designed;
        }
    ...
    self.priorityRegion = regions.priorityRegion(self, shape => shape.width - STEP + self.regionXoffset, () => OFFSET_Y + self.regionYoffset);
    self.infoTypeRegion = regions.infoTypeRegion(self, shape => shape.priorityRegion.getx(shape) - (self.priorityRegion.visible ? STEP : 0) + self.regionXoffset, () => OFFSET_Y + self.regionYoffset);
    self.progressRegion = regions.progressRegion(self, shape => shape.infoTypeRegion.getx(shape) - (self.infoTypeRegion.visible ? STEP : 0) + self.regionXoffset, () => OFFSET_Y + self.regionYoffset);
    self.progressStatusRegion = regions.progressStatusRegion(self, shape => shape.progressRegion.getx(shape) - (self.progressRegion.visible ? STEP : 0) + self.regionXoffset, () => OFFSET_Y + self.regionYoffset);
    self.assignToRegion = regions.assignToRegion(self, shape => shape.progressStatusRegion.getx(shape) - (self.progressStatusRegion.visible ? STEP : 0) + self.regionXoffset, () => OFFSET_Y + self.regionYoffset);
    updateRegionVisibility();
    ...
    // self.serializedFields.batchAdd("emphasized", "priority", "infoType", "progressStatus", "progressPercent", "showedProgress", "emphasizeType", "shineColor1", "shineColor2", "assignTo", "shared", "sharedBy");
...
let text = (id, x, y, width, height, parent, drawer) => {
...
const shapeComment = (context => {
    return (x, y, host, id) => {
        ...
        const COMMENT_HEIGHT = 25, FONT_SIZE = 12;
        ...
        self.bullet = {
            ...
            run: () => {
                ...
                // self.drawer.parent.style.transform = "translate(" + self.bullet.x + "px, " + self.drawer.parent.style.top + ")";
        ...
        self.pop = {
            ...
            run: () => {
                self.drawer.parent.style.top = (host.y + self.page.shapes.count(s => s.loginTag && s.loginTag < self.loginTag) * (self.height + 2)) + "px";
                self.drawer.parent.style.opacity = self.pop.alpha;
                self.pop.alpha -= 0.002;
                if (self.pop.alpha <= 0) {
                    // context.pop.forEach((c, i) => {
                    //     c.drawer.parent.style.top = (host.y + i * (self.height + 2)) + "px";
                    // })
                    self.animate = undefined;
                    self.stop("pop");
                }
        ...
        self.login = () => {
            ...
            const all = self.page.shapes.filter(s => s.loginTag);
            if (all.length === 0) {
                self.loginTag = 1;
            } else {
        ...
        self.run = (mode, comment) => {
            // if (context[mode] === undefined) return;
            // context[mode].remove(c => !self.page.shapes.contains(s => s === c));//try to clear expired comments
            self.invalidate();
            self.login()
            self.page.moveIndexTop(self);
            self.animate = () => {
                // if (context[mode].length < 15 && (!context[mode].contains(c => c === self))) {
                //
                //     context[mode].push(self);
                // }
                // if (!context[mode].contains(c => c === self)) return;//限流了，得等
                if (!self.isInited) {
                    self.isInited = true;
                    self.text = comment;
                    self.autoWidth = true;
        ...
        self.stop = mode => {
            self.logout();
            // context[mode] && context[mode].remove(c => c === self);
            self.stopped();
        };
...
const codeDisplayer = (id, x, y, width, height, parent) => {
    ...
    self.tag = "page.pros";//will be replaced by self.text when editor turned back
