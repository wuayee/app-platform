 ...
 * 辉子 2020
 */
let line = (id, x, y, width, height, parent, drawer) => {
    ...
    self.dragTo = (position) => {
        // 连接shape时，禁止直接拖动线
        if ((self.fromShape !== undefined && self.fromShape !== "") || (self.toShape !== undefined && self.toShape !== "")) {
            return;
        }
    ...
    self.getFromShape = () => self.page.shapes.find(s => s.id === self.fromShape);

    self.getToShape = () => self.page.shapes.find(s => s.id === self.toShape);
    ...
    const initStraightConnectors = () => {
        ...
        cTo.onMouseDrag = (position) => {
            ...
            if ((focusedShapes.length === 1 && focusedShapes[0] === self) || (focusedShapes.length > 1 && !self.toShape)) {
    ...
    const initCurveConnectors = () => {
        const c1 = connector(self, s => self.getCurvePoint1().x, s => self.getCurvePoint1().y, s => DIRECTION.L, s => s.visible && s.lineMode.type === LINEMODE.CURVE.type, s => true, s => false);
        c1.type = "curve begin";
        c1.moving = (deltaX, deltaY) => {
            self.curvePoint1 = {x: self.curvePoint1.x + deltaX, y: self.curvePoint1.y + deltaY};//only first level property change will trigger property change react
            self.invalidateAlone();
        };

        const c2 = connector(self, s => self.getCurvePoint2().x, s => self.getCurvePoint2().y, s => DIRECTION.L, s => s.visible && s.lineMode.type === LINEMODE.CURVE.type, s => true, s => false);
        c2.type = "curve end";
        c2.moving = (deltaX, deltaY) => {
            self.curvePoint2 = {x: self.curvePoint2.x + deltaX, y: self.curvePoint2.y + deltaY};//only first level property change will trigger property change react
            self.invalidateAlone();
        };
    };
    ...
    const generateControlPoints = () => {
        ...
        for (let i = 1; i < self.brokenPoints.length; i++) {
            ...
            if ((controlPointDirection === DIRECTION.H && distanceY < minGenerateControlPointDistance) ||
                (controlPointDirection === DIRECTION.V && distanceX < minGenerateControlPointDistance)) {
                continue;
            }
    ...
    self.follow = () => {
        ...
        if (self.fromShape !== "" && self.fromShape === self.toShape && self.definedFromConnector === "" && self.definedToConnector === "") {
            return;
        }
    ...
    self.containerAllowed = parent => {
         ...
         * update by z00559346 2023.04.10.
         */
        const fromShape = self.getFromShape();
        const toShape = self.getToShape();
    ...
    self.onMessageDataChange = (data) => {
        if (data.hasOwnProperty("fromShape") || data.hasOwnProperty("toShape") || data.hasOwnProperty("definedFromConnector") || data.hasOwnProperty("definedToConnector")) {
            self.follow();
        } else {
            if (self.lineMode === LINEMODE.BROKEN) {
