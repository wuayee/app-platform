...
export const elsaWriter = (graph) => {
    ...
    self.addPage = ({name, id, targetDiv, index, data}) => {
        ...
        // todo@zhangyue presentation需要缩放一点，使其不充满整个画布.暂时规避，后续需要和辉哥讨论(和activePage一起).
    ...
    self.moveUp = (shapes = []) => {
        ...
        const list = shapes.sort((a, b) => b.getIndex() - a.getIndex()).filter(s => {
            const index = s.getIndex();
            if (index === graph.activePage.maxIndex() - 1) {
                return false;
            }
            const nextShape = graph.activePage.shapes[index - Z_INDEX_OFFSET + 1];
            return !shapeIdSet.has(nextShape);
        });
        const result = list.map(s => {
            return {
                shape: s, fromIndex: s.index, toIndex: s.index + 1
            }
        });
        graph.activePage.batchMoveIndexAfter(result);
    };
    ...
    self.moveDown = (shapes = []) => {
        const list = shapes.sort((a, b) => a.getIndex() - b.getIndex()).filter(s => {
            const index = s.getIndex();
            return index !== Z_INDEX_OFFSET;
        });
        const result = list.map(s => {
            return {
                shape: s, fromIndex: s.index, toIndex: s.index - 1
        ...
        graph.activePage.batchMoveIndexBefore(result);
    };
     ...
     */
    self.moveTop = (shapes = []) => {
        const result = shapes.sort((a, b) => b.getIndex() - a.getIndex()).map((s, i) => {
            const MAX_INDEX = graph.activePage.maxIndex() - 1;
            return {
                shape: s, fromIndex: s.index, toIndex: MAX_INDEX - i
            };
        });
        graph.activePage.batchMoveIndexAfter(result);
    };
    ...
    self.moveBottom = (shapes = []) => {
        const result = shapes.sort((a, b) => a.getIndex() - b.getIndex()).map((s, i) => {
            const MIN_INDEX = graph.activePage.getMinIndex();
            return {
                shape: s, fromIndex: s.index, toIndex: MIN_INDEX + i
            };
        });
        graph.activePage.batchMoveIndexBefore(result);
    };
