...
""" 待测试模块 """
from fitframework.core.repo import fitable_register, service_repo, service_repo_utils
...
class FitServiceRepoTest(unittest.TestCase):
    ORIGIN_FOUND_PLUGIN_FOUND_BY_PATH = service_repo_utils._found_plugin_by_path

    @classmethod
    def setUpClass(self):
        service_repo_utils._found_plugin_by_path = mock.Mock(return_value=TEST_PLUGIN_NAME)

    @classmethod
    def tearDownClass(self):
        service_repo_utils._found_plugin_by_path = \
            FitServiceRepoTest.ORIGIN_FOUND_PLUGIN_FOUND_BY_PATH

    def set_up(self, is_bootstrap=True, register_fitable=True):
        if is_bootstrap:
            fitframework.core.servicerepo.fitable_register.loading_context.is_bootstrap = True
        if register_fitable:
            fitable_register.register_fitable(*TEST_GEN_WITH_FITABLE_ID, is_private=False,
                                              fitable_ref=TEST_FITABLE_FUNC)

    def tear_down(self):
        if hasattr(fitframework.core.servicerepo.fitable_register.loading_context, 'is_bootstrap'):
            del fitframework.core.servicerepo.fitable_register.loading_context.is_bootstrap
    ...
    def test_register_fitable_is_boostrap_success(self, mock_register_to_plugin):
        ...
        self.assertEqual(service_repo._fitable_ref_repo[TEST_GEN_WITH_FITABLE_ID],
                         TEST_FITABLE_FUNC)
        self.tear_down()
    ...
    def test_register_fitable_param_error_fail(self):
        with self.assertRaises(AssertionError):
            fitable_register.register_fitable(TEST_GEN_ID, None, is_private=False,
                                              fitable_ref=TEST_FITABLE_FUNC)
        with self.assertRaises(AssertionError):
            fitable_register.register_fitable('', TEST_GEN_FITABLE_ID, is_private=False,
                                              fitable_ref=TEST_FITABLE_FUNC)
