...
from fit_common_struct.core import Address as FitAddress
from fit_common_struct.core import Endpoint, FitableInfo
from fit_common_struct.registry_v1 import AddressForRegistryV1, FitableForRegistryV1
...
from fitframework.utils.tools import to_list, get_local_ip
...
@local_context('local_ip', get_local_ip())
def local_ip():
    pass
...
def get_fit_service_address_list(fitable: FitableForRegistryV1) -> List[AddressForRegistryV1]:
    pass
...
@private_fit
def is_address_valid(address: AddressForRegistryV1):
    pass
...
class ConfigureBasedBroker(BrokerTemplate, metaclass=Singleton):
    ...
    def default_routing(self, generic_id, fitable_identifier, fit_ref, *args) -> List[str]:
        ...
        fitable_ids = get_all_fitables_from_registry(generic_id)
    ...
    def default_load_balancing(self, generic_id, fitable_id, fitable_info):
        ...
        if address or _is_force_local(generic_id):
            ...
            return address
        endpoints = self.get_fit_service_endpoint(fitable_info)
        fit_logger.debug(f"load balance by default, get remote target, gid: {generic_id}, fid: {fitable_id}, "
                         f"endpoints count: {len(endpoints)}")
        if len(endpoints) == 0:
            fit_logger.warning(f"cannot get any endpoint can use in this worker. "
                               f"[genericable_id={fitable_info.genericableId}, fitable_id={fitable_info.fitableId}]")
            return None
        # no choice!
        if len(endpoints) == 1:
            return endpoints[0]

        return _load_balancing(fitable_info, endpoints)

    def get_fit_service_endpoint(self, fitable_info: FitableInfo) -> List[Endpoint]:
        """
            获得所有fitable地址列表
            先根据支持的客户端通讯规约/协议过滤
            再根据环境标和环境链过滤
        ...
        """
        endpoints = _get_fit_service_address_with_priorities(fitable_info)
        if not endpoints:
            fit_logger.warning(f"cannot get any endpoint after checking format and protocol. "
                               f"[genericable_id={fitable_info.genericableId}, fitable_id={fitable_info.fitableId}]")
            return []

        endpoints = _load_balance_env_filtering(fitable_info, endpoints)
        if not endpoints:
    ...
    @print_trace
    def fit_execute(self, address, generic_id: str, fitable_id: str, fit_ref, *args, timeout) -> Any:
        ...
        if isinstance(address, Endpoint):
            ...
            fit_invoke_info = (generic_id, fitable_id, fit_ref)
            return remote_invoker.call(address, fit_invoke_info, args, timeout)
        else:
            return address(*args)
    ...
    def custom_load_balancing(self, address_filter, fitable_info: FitableInfo):
        # 获得所有fitable地址列表
        # 先根据支持的客户端通讯规约/协议过滤
        endpoints = self.get_fit_service_endpoint(fitable_info)
        if len(endpoints) == 0:
            ...
            return None
        try:
            endpoints = [endpoint for endpoint in endpoints if address_filter(endpoint)]
        except Exception as exception:
            fit_logger.error(f'filter address error, msg: {exception}')
            return None
        if not endpoints:
            fit_logger.warning(f"cannot get any endpoint after custom load balancing. "
                               f"[genericable_id={fitable_info.genericableId}, fitable_id={fitable_info.fitableId}]")
            return None
        if len(endpoints) > 1:
            fit_logger.warning(f"get more than one endpoint after custom load balancing. "
                               f"[genericable_id={fitable_info.genericableId}, fitable_id={fitable_info.fitableId}]")
            return endpoints[0]

        if endpoints[0].address.workerId == _worker_id():
            return service_repo.get_fitable_ref(fitable_info.genericableId, fitable_info.fitableId)
...
def _get_local_address():
    return AddressForRegistryV1(local_ip(), 0, '', 0, [], worker_env())


def _get_local_endpoint():
    return _build_endpoint_from_address(_get_local_address())


def _load_balance_env_filtering(fitable: FitableForRegistryV1, endpoints: List[Endpoint]) -> List[Endpoint]:
    """ 负载均衡环境筛选 """

    def lb_filter_template(_endpoints: List[Endpoint]) -> List[Endpoint]:
        pass
...
def _load_balancing(fitable: FitableInfo, endpoints: List[Endpoint]) -> AddressForRegistryV1:
    """ 负载均衡 """

    def lb_call_template(fitable_info: FitableInfo, caller_endpoints: Endpoint,
                         target_endpoints: List[Endpoint]) -> AddressForRegistryV1:
        pass

    args = fitable, _get_local_endpoint(), endpoints
...
def _get_fit_service_address_with_priorities(fitable_info: FitableInfo) -> List[Endpoint]:
    endpoints = [addr for addr in _get_fit_service_address_and_convert(fitable_info) if _is_supported_address(addr)]
    return _remove_duplicated_ids(endpoints)


def _get_fit_service_address_and_convert(fitable_info: FitableInfo):
    address_list = get_fit_service_address_list(
        FitableForRegistryV1(fitable_info.genericableId, const.FIXED_GENERICABLE_VERSION, fitable_info.fitableId,
                             const.FIXED_FITABLE_VERSION))
    fit_logger.debug(f"got address, gid: {fitable_info.genericableId}, count: {len(address_list)}")
    endpoints = [_build_endpoint_from_address(address) for address in address_list]
    if len(endpoints) == 0:
        fit_logger.warning(f"cannot get any endpoint by registry client. "
                           f"[genericable_id={fitable_info.genericableId}, fitable_id={fitable_info.fitableId}]")
    return endpoints
...
def _build_endpoint_from_address(address: AddressForRegistryV1):
    return Endpoint(FitAddress(workerId=address.id, host=address.host, port=address.port),
                    ...
                    weight=_calculate_weight(address))
...
def _is_supported_address(address: Endpoint) -> bool:
    from fitframework.core.broker import remote_invoker
    return address.protocol in get_registered_clients() and remote_invoker.get_supported_formats(
        address.serializeFormats)


def _build_address_id(endpoint: Endpoint) -> str:
    return f"{endpoint.address.host}:{endpoint.address.port}"


def _remove_duplicated_ids(endpoints: List[Endpoint]):
    from fitframework.core.broker import remote_invoker

    def sort_rule(_address):
        return (_build_address_id(_address),
                get_priority_index(_address.protocol, _address.serializeFormats,
                                   remote_invoker.protocol_priors()))

    endpoints.sort(key=sort_rule)
    result = []
    current_id = None
    for endpoint in endpoints:
        if _build_address_id(endpoint) != current_id:
            result.append(endpoint)
            current_id = endpoint.address.workerId
    return result
