...
class BrokerTemplate:
    ...
    def _is_routing_error_or_load_balance_error(err: FitException):
        return err.error_code == InternalErrorCode.ROUTING_ERROR.value or \
            err.error_code == InternalErrorCode.LOAD_BALANCING_ERROR.value

    def fit_ffp_invoke(self, generic_id: str, fitable_identifier: FitableIdentifier, fit_ref, *args, timeout, retry,
                       route_filter, address_filter):
        try:
            ...
            result = self.fit_invoke(generic_id, fitable_identifier, fit_ref, *args, timeout=timeout, retry=retry,
                                     route_filter=route_filter, address_filter=address_filter)
    ...
    def fit_execute(self, address, generic_id, fitable_id, fit_ref, *args, timeout):
        """ 执行Fitable instance
        """
        pass
    ...
    def fit_invoke(self, generic_id: str, fitable_identifier: FitableIdentifier, fit_ref, *args, timeout, retry,
                   route_filter, address_filter):
        @degradable
        @exception_handler
        @retryable(retry_times=retry)
        def fit_lb_and_execute(broker, generic_id, fitable_id, fit_ref, *args, timeout, address_filter) -> Any:
            ...
            if address is None:
                raise FailedLoadBalancingException(generic_id, fitable_id)
        ...
        for the_id in work_fit_ids:
            ...
            result = fit_lb_and_execute(self, generic_id, the_id, fit_ref, *args, timeout=timeout,
