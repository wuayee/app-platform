...
from fit_common_struct.core import Endpoint, FitableTrace, BaseFitTrace
...
from fitframework.utils.tools import get_local_ip
...
class TracePrint:
    ...
    def __init__(self, address, generic_id, fitable_id, fit_ref):
...
def get_target_address(address_):
    return (address_.address.host, address_.address.port) \
        if isinstance(address_, Endpoint) else ('', '')
