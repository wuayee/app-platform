...
_STARTUP_COMP_KEY = 'comp_folder'
_STARTUP_THIRD_PARTY_KEY = 'third_party_folder'
...
# 框架版本
_FIT_FRAMEWORK_VERSION = "1.0.2.dev"
...
def bootstrap_start(sys_plugins: List[str], user_plugins, config_folder: str) -> bool:
    pass
...
def _add_dependency_paths():
    multi_maps_exhaust((_STARTUP_COMP_KEY, _STARTUP_THIRD_PARTY_KEY),
                       runtime_context.get_item, _full_path, sys.path.append)


def parse_arguments():
    parser = argparse.ArgumentParser()
    parser.add_argument(dash_style(_STARTUP_COMP_KEY),
                        help='Root folders to load common structs. default is `fit_common_struct` directory.')
    parser.add_argument(dash_style(_STARTUP_THIRD_PARTY_KEY),
                        help='Root folders to load third parties. default is `third_party` directory.')
...
@fit("com.huawei.fit.get.should.terminate.main")
...
def main():
    ...
    bootstrap_start(plugins_info.get(const.STARTUP_PLUGINS_SYSTEM_KEY, []),
                    plugins_info.get(const.STARTUP_PLUGINS_USER_KEY, []),
