...
def value(key: str, default_value=None, converter=None):
    ...
    优先级序列如下：
        进程入参配置：runtime_context[${key}]
            - 用户入参
            - 环境变量
            - 进程入参的默认值
        全局fit_startup.yml：fit_config_repo['__global__'][${key...}]
        插件配置：fit_config_repo[${plugin_of_func}][${key...}]
        配置默认值：default_value
...
def local_context(key: str, default_value=None, converter=None):
    """
    装饰器：环境或进程入参配置注入，配置缺失取default_value。都没有配，返回None
    优先级序列如下：
       环境或进程入参
       全局fit_startup.yml配置
       默认值
    ...
    def decorator(func):
        ...
        def wrapper(*_):
            val = runtime_context.get_item(key)
            if val is None:
                from fitframework.core.repo.service_repo import get_configuration_item
                val = get_configuration_item(const.GLOBAL_PLUGIN_NAME, key)
            if val is None:
                return default_value
            return converter(val) if converter else val
