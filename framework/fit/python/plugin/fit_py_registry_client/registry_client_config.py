...
def _build_address_from_dict(addr: str) -> AddressForRegistryV1:
    ...
    ip, port = addr.split(':')
    return AddressForRegistryV1(host=ip, port=int(port), protocol=_registry_server_protocol(),
                                environment=_worker_env(), formats=_registry_server_formats(), id='',
                                context_path=_registry_context_path())
