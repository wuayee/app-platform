...
public class RemoteFitableExecutor extends AbstractUnicastFitableExecutor {
    ...
    protected Object execute(Fitable fitable, Target target, InvocationContext context, Object[] args) {
        ...
        RequestMetadataV2 requestMetadataBytes = this.getRequestMetadataBytes(format.code(), fitable);
    ...
    private FitException buildException(Fitable fitable, ResponseMetadataV2 responseMetadataV2) {
        Optional<Class<? extends FitException>> opClass = Errors.exceptionClass(responseMetadataV2.code());
        ...
        if (opClass.isPresent()) {
            ...
            exception = ReflectionUtils.instantiate(declaredConstructor, responseMetadataV2.message());
        } else {
            Map<Integer, Class<? extends FitException>> errorCodes = this.errorCodesLoader.get();
            if (errorCodes.containsKey(responseMetadataV2.code())) {
                Class<? extends FitException> responseExceptionClass = errorCodes.get(responseMetadataV2.code());
                Constructor<? extends FitException> declaredConstructor =
                        ReflectionUtils.getDeclaredConstructor(responseExceptionClass, String.class);
                exception = ReflectionUtils.instantiate(declaredConstructor, responseMetadataV2.message());
            } else {
                exception = new FitException(responseMetadataV2.code(), responseMetadataV2.message());
            }
        }
        byte[] propertiesBytes = responseMetadataV2.tagValues().getValue(ValueSerializer.TAG_EXCEPTION_PROPERTIES);
    ...
    private RequestMetadataV2 getRequestMetadataBytes(int format, Fitable fitable) {
        return RequestMetadataV2.custom()
                .version(RequestMetadataV2.CURRENT_VERSION)
    ...
    private boolean isSuccess(ResponseMetadataV2 responseMetadataV2) {
        return responseMetadataV2.code() == ResponseMetadataV2.CODE_OK;
    }

    private Response requestResponse(Target target, InvocationContext context, RequestMetadataV2 metadata,
            Object[] args, Method method) {
