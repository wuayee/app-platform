...
public class DefaultJsonSchemaManager implements JsonSchemaManager {
    ...
    public DefaultJsonSchemaManager(@Fit(alias = "json") ObjectSerializer jsonSerializer) {
    ...
    public JsonSchema createSchema(Method method) {
        ...
        ObjectSchema parameterSchema = new ObjectSchema(Map.class, this.jsonSerializer);
        for (Parameter parameter : method.getParameters()) {
            ...
            parameterSchema.addSchema(new DecoratedSchema(name, description, schema, this.jsonSerializer), required);
        }
    ...
    private JsonSchema prepareInitialSchema(Type type) {
        if (SchemaTypeUtils.isArrayType(type)) {
            return new ArraySchema(type, this.jsonSerializer);
        }
        if (SchemaTypeUtils.isObjectType(type)) {
            return new ObjectSchema(type, this.jsonSerializer);
        }
        return JsonSchema.createPrimitive(type, this.jsonSerializer);
    }
}
