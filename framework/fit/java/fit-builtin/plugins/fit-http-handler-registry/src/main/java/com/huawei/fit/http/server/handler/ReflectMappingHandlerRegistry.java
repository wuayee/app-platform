 ...
 */
@Component
public class ReflectMappingHandlerRegistry extends AbstractHandlerResolver
        implements PluginStartedObserver, PluginStoppingObserver {
    private static final Logger log = Logger.get(ReflectMappingHandlerRegistry.class);
     ...
     */
    public ReflectMappingHandlerRegistry(BeanContainer container, HttpHandlerResolver handlerResolver,
    ...
    private void registerHttpHandlerGroup(HttpHandlerGroup group,
        ...
        for (Map.Entry<Method, List<HttpHandler>> entry : group.getMethodHandlersMapping().entrySet()) {
            ...
            for (HttpHandler handler : entry.getValue()) {
                if (handler instanceof DefaultReflectMappingHandler) {
                    DefaultReflectMappingHandler actualHandler = cast(handler);
                ...
                methodNames.forEach(methodName -> this.server.dispatcher().register(methodName, handler));
            }
        }
        this.server.dispatcher().registerGroup(group);
    ...
    private void unregisterHttpHandlerGroup(HttpHandlerGroup group, List<HttpMethodNameResolver> methodNameResolvers) {
        for (Map.Entry<Method, List<HttpHandler>> entry : group.getMethodHandlersMapping().entrySet()) {
            ...
            for (HttpHandler handler : entry.getValue()) {
                ...
                methodNames.forEach(methodName -> this.server.dispatcher().unregister(methodName, handler));
            }
        }
        this.server.dispatcher().unregisterGroup(group.getName());
    ...
    private List<HttpHandlerGroup> getMethodAndHttpHandlers(Plugin plugin) {
        ...
        GlobalPathPatternPrefixResolver pathPatternPrefixResolver = this.getPathPatternPrefixResolver(container);
    ...
    private GlobalPathPatternPrefixResolver getPathPatternPrefixResolver(BeanContainer container) {
