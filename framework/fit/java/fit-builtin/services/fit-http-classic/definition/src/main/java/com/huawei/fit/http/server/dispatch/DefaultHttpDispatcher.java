...
public class DefaultHttpDispatcher implements HttpDispatcher {
     ...
     *         <li>映射中的值表示 Http 请求路径的匹配树的 {@link MappingTree}。</li>
     *     </ul>
     * </p>
     */
    private final Map<HttpRequestMethod, MappingTree> pathVariableHandlers = new ConcurrentHashMap<>();
    ...
    public HttpHandler dispatch(HttpClassicServerRequest request, HttpClassicResponse response) {
        return OptionalUtils.get(() -> this.selectFromNoPathVariableHandlers(request))
                ...
                .orElseThrow(() -> {
                    String message = StringUtils.format("No handler for http request. [method={0}, path={1}]",
                            request.method().name(),
                            request.path());
    ...
    private Optional<HttpHandler> selectFromPathVariableHandlers(HttpClassicServerRequest request) {
        MappingTree mappingTree = this.pathVariableHandlers.get(request.method());
        if (mappingTree == null) {
            return Optional.empty();
        }
    ...
    public void register(String httpMethod, HttpHandler handler) {
        ...
        String pathPattern = this.convertToMatchedPathPattern(handler.pathPattern());
    ...
    public Map<HttpRequestMethod, List<HttpHandler>> getHttpHandlersMapping() {
        ...
        this.pathVariableHandlers.forEach((requestMethod, mappingTree) -> {
            ...
            handlerList.addAll(mappingTree.getAllHttpHandlers());
        });
    ...
    private String convertToMatchedPathPattern(String pathPattern) {
