 ...
 * @since 2022-07-28
 */
public abstract class AbstractReflectMappingHandler extends AbstractHttpHandler implements ReflectibleMappingHandler {
    ...
    /**
     * 通过 Http 处理的相关静态信息和执行信息来实例化 {@link AbstractReflectMappingHandler}。
     *
     ...
     * 为 {@code null} 时。
     */
    protected AbstractReflectMappingHandler(StaticInfo staticInfo, ExecutionInfo executionInfo) {
        ...
        this.target = notNull(executionInfo.target(), "The bean to invoke after parameter mapping cannot be null.");
        this.method = notNull(executionInfo.method(),
                "The specified method to invoke after parameter mapping cannot be null.");
    ...
    public void handle(HttpClassicServerRequest request, HttpClassicServerResponse response)
        ...
        Object[] args;
        try {
            args = this.propertyValueMappers.stream().map(httpMapper -> httpMapper.map(request, response)).toArray();
