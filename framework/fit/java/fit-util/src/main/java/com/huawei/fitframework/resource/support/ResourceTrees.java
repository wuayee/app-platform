...
public final class ResourceTrees {
    private static final WeakCache<Jar, ResourceTree> JARS = WeakCache.create(
            JarResourceTree::new, JarResourceTree::jar);
    private static final WeakCache<File, ResourceTree> DIRS = WeakCache.create(
            DirectoryResourceTree::new, DirectoryResourceTree::directory);
        ...
        if (location.nests().isEmpty() && location.file().isDirectory()) {
            ...
            try {
                ...
                throw new IllegalStateException(StringUtils.format(
                        "Failed to load JAR of resource tree. [url=%s]",
