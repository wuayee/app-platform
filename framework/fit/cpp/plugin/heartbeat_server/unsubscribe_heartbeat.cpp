 ...
 * Notes        :
 */
#include <genericable/com_huawei_fit_heartbeat_unsubscribe_heartbeat/1.0.0/cplusplus/unsubscribeHeartbeat.hpp>
...
namespace fit {
namespace heartbeat {
static FitCode UnsubscribeHeartbeatImpl(void *ctx,
    ...
    bool **ret)
{
    ...
    for (auto &item : innerAddressBeatInfos) {
        auto retCode = fit_heartbeat_manager::instance()->get_notify_service().unsubscribe(item);
        if (retCode != FIT_ERR_SUCCESS) {
            FIT_LOG_ERROR(
                "Unsubscribe has error. ret = %d, id = %s, scene = %s, callback fitid = %s, callback address = [%s].",
            ...
            FIT_LOG_INFO("Unsubscribe success. id = %s, scene = %s, callback fitid = %s, callback address = [%s].",
...
FIT_REGISTRATIONS
{
    Fit::Framework::Annotation::Fitable(UnsubscribeHeartbeatImpl)
        .SetGenericId(fit::heartbeat::unsubscribeHeartbeat::GENERIC_ID)
        .SetFitableId("unsubscribe_heartbeat_cpp");
}
}
}
