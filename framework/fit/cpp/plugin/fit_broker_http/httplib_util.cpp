...
HttpResult HttplibUtil::GetRequest(const httplib::Request& req, Network::Request& innerReq)
{
    ...
    innerReq.meta = fit_meta_data(
        version, format, fit_version::parse_from(req.get_header_value(HEADER_FIT_GENERICABLE_VERSION), {1, 0, 0}),
        paths[0], paths[1], req.get_header_value(HEADER_FIT_ACCESS_TOKEN).c_str());
    ...
    if (!tlvBase64Bytes.empty() && (tlvBase64Bytes.size() / BASE64_ENCODE_SIZE_UNIT == 0)) {
        innerReq.meta.Deserialize(Base64Decode(to_fit_string(tlvBase64Bytes)));
    }
