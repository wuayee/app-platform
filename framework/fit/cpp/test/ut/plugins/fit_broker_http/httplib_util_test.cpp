...
TEST(FitBrokerHttplibUtilTest, should_return_ok_when_GetRequest_given_correct_request)
{
    auto meta = fit_meta_data(3, 1, fit_version(1, 0, 1), "9289a2a4322d47d38f33fc32c47f04d2", "fitableId");
    auto expectMetaBytes = fit_meta_package_builder::build(meta);
    ...
    EXPECT_EQ(req.metadata, expectMetaBytes);
    EXPECT_EQ(req.payload, expectPayloadBytes);
}

TEST(FitBrokerHttplibUtilTest, should_return_illegal_fit_path_when_GetRequest_given_large_path_layer)
{
    httplib::Request httpReq;
    std::regex reg(R"(/fit/(.*))");
...
TEST(FitBrokerHttplibUtilTest, should_return_illegal_fit_path_when_GetRequest_given_empty_match_size)
{
    httplib::Request httpReq;
    Network::Request req {};
    auto ret = HttplibUtil::GetRequest(httpReq, req);
...
TEST(FitBrokerHttplibUtilTest, should_return_no_format_header_when_GetRequest_given_no_format_header)
{
    httplib::Request httpReq;
    Network::Request req {};
    std::regex reg(R"(/fit/(.*))");
...
TEST(FitBrokerHttplibUtilTest, should_return_ok_when_GetRequest_given_no_version_header)
{
    httplib::Request httpReq;
    Network::Request req {};
    std::regex reg(R"(/fit/(.*))");
...
TEST(FitBrokerHttplibUtilTest, should_return_ok_when_GetRequest_given_no_genericable_version_header)
{
    httplib::Request httpReq;
    Network::Request req {};
    std::regex reg(R"(/fit/(.*))");
...
TEST(FitBrokerHttplibUtilTest, should_return_msg_when_BuildExceptionResponse_given_params)
{
    httplib::Request httpReq;
...
TEST(FitBrokerHttplibUtilTest, should_return_correct_headers_when_BuildRequestHeaders_given_req_meta)
{
    auto meta = fit_meta_data(3, 1, fit_version(1, 0, 1), "9289a2a4322d47d38f33fc32c47f04d2", "fitableId");
