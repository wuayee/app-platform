...
class PassiveAddressSynchronizerTest : public ::testing::Test {
public:
    void SetUp() override
    {
...
TEST_F(PassiveAddressSynchronizerTest, should_return_void_when_start_given_empty)
{
    // given
    ...
    EXPECT_CALL(*spi_, SubscribeFitablesChanged(testing::_))
        .Times(testing::AtLeast(1))
        .WillRepeatedly(::testing::Invoke([](FitablesChangedCallbackPtr) {
            return;
        }));
    EXPECT_CALL(*spi_, UnsubscribeFitablesChanged(testing::_))
        .Times(testing::AtLeast(1))
        .WillRepeatedly(::testing::Invoke([](FitablesChangedCallbackPtr) {
            return;
        }));
    // when
    PassiveAddressSynchronizer passiveAddressSynchronizer(registryListener_);
    passiveAddressSynchronizer.Start();
    passiveAddressSynchronizer.Stop();
    // then
}

TEST_F(PassiveAddressSynchronizerTest, should_return_void_when_start_given_fitable_instance)
{
    // given
    ...
    EXPECT_CALL(*spi_, SubscribeFitables(testing::A<const vector<FitableInfo>&>()))
        .Times(testing::AtLeast(1))
        .WillRepeatedly(::testing::Invoke([subscribeFitableInstances](const vector<FitableInfo>&) {
            return FitableInstanceListGuard(subscribeFitableInstances, FIT_OK);
        }));
    EXPECT_CALL(*spi_, SubscribeFitablesChanged(testing::_))
        .Times(testing::AtLeast(1))
        .WillRepeatedly(::testing::Invoke([](FitablesChangedCallbackPtr) {
