...
class MemoryFitableApplicationRepoTest : public ::testing::Test {
public:
    void SetUp() override
    {
...
TEST_F(MemoryFitableApplicationRepoTest, should_return_registry_listener_when_get_listener_given_empty)
{
    // given
    FitablePtr fitablePtr = registryListener_->GetFitable(fitableInfo_, true);
    FitableApplicationRepoPtr fitableApplicationRepoPtr = fitablePtr->GetApplications();
    // when
...
TEST_F(MemoryFitableApplicationRepoTest, should_return_application_fitable_when_get_given_application)
{
    // given
    FitablePtr fitablePtr = registryListener_->GetFitable(fitableInfo_, true);
    ...
    FitableApplicationRepoPtr fitableApplicationRepoPtr = fitablePtr->GetApplications();
    // when
    ApplicationFitablePtr applicationFitablePtr = fitableApplicationRepoPtr->Get(applicationPtr, true);
    // then
    EXPECT_EQ(applicationFitablePtr != nullptr, true);
}

TEST_F(MemoryFitableApplicationRepoTest,
    should_return_application_when_get_and_remove_application_given_application)
{
    // given
    FitablePtr fitablePtr = registryListener_->GetFitable(fitableInfo_, true);
    ApplicationPtr applicationPtr =
    ...
    FitableApplicationRepoPtr fitableApplicationRepoPtr = fitablePtr->GetApplications();
    // when
    ApplicationFitablePtr getApplicationFitablePtrBefore = fitableApplicationRepoPtr->Get(applicationPtr, true);
    ApplicationFitablePtr removeApplicationFitablePtr = fitableApplicationRepoPtr->Remove(applicationPtr);
    ApplicationFitablePtr getApplicationFitablePtr = fitableApplicationRepoPtr->Get(applicationPtr, false);
    // then
    EXPECT_EQ(getApplicationFitablePtrBefore != nullptr, true);
