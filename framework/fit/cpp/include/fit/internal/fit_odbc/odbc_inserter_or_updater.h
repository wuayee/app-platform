...
#ifndef ODBC_INSERTER_OR_UPDATER_H
#define ODBC_INSERTER_OR_UPDATER_H
...
class insert_or_update_sql_builder {
public:
    insert_or_update_sql_builder &bind(const Fit::string &table)
    {
        table_ = table;
        return *this;
    }

    insert_or_update_sql_builder &insert(const Fit::string &column_name, const Fit::string &value)
    {
        insert_columns_.push_back(make_pair(column_name, value));
        return *this;
    }

    insert_or_update_sql_builder &update(const Fit::string &column_name, const Fit::string &value)
    {
...
class odbc_inserter_or_updater {
public:
    explicit odbc_inserter_or_updater(odbc_stmt_ptr stmt) : stmt_(std::move(stmt)) {}

    ~odbc_inserter_or_updater() = default;

    odbc_inserter_or_updater &bind(const Fit::string &table)
    {
    ...
    odbc_inserter_or_updater &insert(const Fit::string &col_name, const __VALUE_TYPE &value)
    {
        sql_builder_.insert(col_name, Fit::to_string(value));
        return *this;
    }

    odbc_inserter_or_updater &insert(const Fit::string &col_name, const Fit::string &value)
    {
        sql_builder_.insert(col_name, '\'' + value + '\'');
        return *this;
    }

    odbc_inserter_or_updater &insert_timestamp(const Fit::string &col_name, const time_t &value)
    {
    ...
    template<typename __VALUE_TYPE>
    odbc_inserter_or_updater &update(const Fit::string &col_name, const __VALUE_TYPE &value)
    {
        sql_builder_.update(col_name, Fit::to_string(value));
        return *this;
    }

    odbc_inserter_or_updater &update(const Fit::string &col_name, const Fit::string &value)
    {
        sql_builder_.update(col_name, '\'' + value + '\'');
        return *this;
    }

    odbc_inserter_or_updater &update_timestamp(const Fit::string &col_name, const time_t &value)
    {
        Fit::string render_value = Fit::TimeUtil::to_string<Fit::TimeUtil::normal_utc_time>(value);
...
private:
    odbc_stmt_ptr stmt_;
    insert_or_update_sql_builder sql_builder_;
};
