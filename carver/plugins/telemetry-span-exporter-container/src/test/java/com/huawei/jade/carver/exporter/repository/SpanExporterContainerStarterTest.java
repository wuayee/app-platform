/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2024-2024. All rights reserved.
 */

package com.huawei.jade.carver.exporter.repository;

import static org.assertj.core.api.Assertions.assertThat;

import com.huawei.fitframework.annotation.Fit;
import com.huawei.fitframework.plugin.Plugin;
import com.huawei.fitframework.test.annotation.FitTestWithJunit;
import com.huawei.fitframework.util.ObjectUtils;
import com.huawei.jade.carver.exporter.container.DefaultSpanExporterContainer;
import com.huawei.jade.carver.exporter.container.SpanExporterContainerStarter;
import com.huawei.jade.carver.exporter.container.SpanProcessorConfig;
import com.huawei.jade.carver.exporter.repository.stub.SpanExporterSub;
import com.huawei.jade.service.CarverGlobalOpenTelemetry;
import com.huawei.jade.service.SpanExporterContainer;

import io.opentelemetry.api.OpenTelemetry;

import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Nested;
import org.junit.jupiter.api.Test;

import java.lang.reflect.Field;

/**
 * {@link SpanExporterContainerStarter} 的测试。
 *
 * @author 刘信宏
 * @since 2024-07-25
 */
public class SpanExporterContainerStarterTest {
    private SpanExporterContainer getExporterContainer(SpanExporterContainerStarter containerStarter)
            throws NoSuchFieldException, IllegalAccessException {
        Field exportersContainer = containerStarter.getClass().getDeclaredField("exportersContainer");
        exportersContainer.setAccessible(true);
        return ObjectUtils.cast(exportersContainer.get(containerStarter));
    }

    @FitTestWithJunit(classes = {SpanExporterContainerStarter.class, DefaultSpanExporterContainer.class,
            SpanExporterSub.class, SpanProcessorConfig.class})
    @Nested
    @DisplayName("测试插件带有 SpanExporter 的场景")
    class ContainerStarterWithExporterTest {
        @Fit
        private SpanExporterContainerStarter containerStarter;
        @Fit
        private Plugin plugin;

        @Test
        @DisplayName("注册/注销 exporter 成功。")
        public void shouldOkWhenRegisterExporterAfterRegistering()
                throws NoSuchFieldException, IllegalAccessException {
            // 插件启动，自动触发一次 onPluginStarted
            assertThat(CarverGlobalOpenTelemetry.get()).isNotEqualTo(OpenTelemetry.noop());

            SpanExporterContainer exporterContainer = getExporterContainer(this.containerStarter);
            assertThat(exporterContainer.get(exporter -> true).size()).isEqualTo(1);

            this.containerStarter.onPluginStopping(this.plugin);
            assertThat(exporterContainer.get(exporter -> true).size()).isEqualTo(0);

            this.containerStarter.onPluginStarted(this.plugin);
            assertThat(CarverGlobalOpenTelemetry.get()).isNotEqualTo(OpenTelemetry.noop());
            assertThat(exporterContainer.get(exporter -> true).size()).isEqualTo(1);
        }
    }

    @FitTestWithJunit(classes = {SpanExporterContainerStarter.class, DefaultSpanExporterContainer.class,
            SpanProcessorConfig.class})
    @Nested
    @DisplayName("测试插件无 SpanExporter 的场景")
    class ContainerStarterWithoutExporterTest {
        @Fit
        private SpanExporterContainerStarter containerStarter;

        @Test
        @DisplayName("插件初始化，无 exporter，手动注册 exporter 成功。")
        public void shouldOkWhenPluginInitWithoutExporter()
                throws NoSuchFieldException, IllegalAccessException {
            assertThat(CarverGlobalOpenTelemetry.get()).isEqualTo(OpenTelemetry.noop());

            SpanExporterContainer exporterContainer = getExporterContainer(this.containerStarter);
            assertThat(exporterContainer.get(exporter -> true).size()).isEqualTo(0);

            exporterContainer.register(new SpanExporterSub());
            assertThat(exporterContainer.get(exporter -> true).size()).isEqualTo(1);
            assertThat(CarverGlobalOpenTelemetry.get()).isNotEqualTo(OpenTelemetry.noop());
        }
    }
}
